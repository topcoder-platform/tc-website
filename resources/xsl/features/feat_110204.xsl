...
<br/><br/>
In a key exchange system, two parties need to agree upon an encryption key in such a way that even if a third party intercepts all of their communication, the key cannot be reconstructed.  One such method is "Exponential Key Exchange".  The basic idea is to pick a very large prime number n, and a number g, which is a generator for n.  (A generator is a number whose powers cycle through all possible values mod n.)  Each party generates, on their own, a random number, call them a and b.  From those values, generate <i>A = g<sup>a</sup> (</i>MOD<i> n), B = gb (MOD n)</i>.  <i>A</i> and <i>B</i> are then exchanged.  The first party calculates <i>B<sup>a</sup> (</i>MOD<i> n) = g<sup>ab</sup></i>.  The other party likewise calculates <i>A<sup>b</sup> (</i>MOD<i> n) = g<sup>ab</sup></i>.  <i>g<sup>ab</sup> (</i>MOD<i> n)</i> then becomes the agreed upon key.  This works because even given <i>g<sup>a</sup> (</i>MOD<i> n)</i> and <i>g<sup>b</sup> (</i>MOD<i> n)</i>, there is no easy way to calculate <i>g<sup>ab</sup></i> without knowing <i>a</i> or <i>b</i>.  This is the discrete logarithm problem, and is hard problem.
<br/><br/>
This is very similar to public-key encryption.  The principle is to generate a pair of corresponding keys, one public-for encrypting messages, and another private-for decrypting messages.  The trick here is to generate the key pair in such a way that the private key cannot be easily deduced from the public key.  The most common algorithms for this involve picking two very large prime numbers <i>p</i> and <i>q</i>, letting <i>n = pq</i>, and generating a key pair, <i>d</i> and <i>e</i>, based upon <i>p</i>, <i>q</i>, and <i>n</i>.  (The exact mathematics is a bit complicated to get into here.)  Then, <i>d</i> and <i>n</i> are the public key; <i>e</i> and <i>n</i> are the private key; and discard <i>p</i> and <i>q</i>.  If <i>M</i> is the input data, then the encrypted data is <i>E = Md (</i>MOD<i> n)</i>, and decryption is accomplished by <i>M = Ee (</i>MOD<i> n)</i>.  With <i>p</i> and <i>q</i> discarded, reverse engineering the public key breaks down to the problem of factoring a very large number, which, like the discrete logarithm problem, is hard problem.
<br/><br/>
