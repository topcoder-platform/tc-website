<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template name="big_promo">
            <A href="/index?t=tournaments&amp;c=tco03_highlights" target="_top"><img src="/i/tournament/tco03/promo_congrats_qualifiers.gif" alt="Congratulations to all qualifying members" width="453" height="35" border="0" /></A><br/>

            <A href="/i/tournament/tco03/tco03_tshirt_design.gif" target="_blank"><img src="/i/tournament/tco03/promos_tshirt.gif" alt="Top 500 qualifiers will win a 2003 TCO t-shirt -- View the design" width="340" height="25" border="0" /></A><br/>

            <A href="/index?t=tournaments&amp;c=tco03_overview" target="_top"><img src="/i/tournament/tco03/tco_home_banner.gif" alt="2003 TopCoder Open sponsord by Intel&#174;" width="540" height="125" border="0" /></A><br/>

            <img src="/i/clear.gif" alt="" width="10" height="5" border="0" /><br/>

            <A href="/index?t=tournaments&amp;c=tco03_rules_eligibility" target="_top"><img src="/i/promos/worldwide_prizes_promo.gif" alt="TopCoder Open prize money now available worldwide!" width="510" height="28" border="0" /></A><br/>
  </xsl:template>
</xsl:stylesheet>
