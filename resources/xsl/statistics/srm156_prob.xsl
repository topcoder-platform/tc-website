...
<p>At the end of the challenge phase, it seemed certain that <b>Yarin</b> would win tonight's SRM by over 100 points.  However, 
...
consecutive matches, crushing <b>Yarin</b>'s almost one-year-old record of 7.</p>

<p>The race for top score in Division-II was a bit less intense, as most coders who breezed through the easy and medium problems were 
...
Used as: Division Two - Level One: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>derelikt</b> for 245.89 points (3 mins 41 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
<p>Your goal here is to calculate the smallest number of hard drives your scattered files can take up, given the capacity and current use of 
each drive.</p>
<p>The first step is to determine how much data there is total between your various hard drives, by adding together the amounts on each drive. 
...
unallocated data. At this point, the greedy algorithm is actually a very good strategy, as the best way to minimize number of drives used is to 
...
least, subtracting from your data total until it reaches or passes zero.</p>
...
Used as: Division Two - Level Two: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>derelikt</b> for 559.45 points (7 mins 45 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
Used as: Division One - Level One: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>SnapDragon</b> for 294.46 points (3 mins 54 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
<p>Here you must solve a formula to give the likelihood that you will win on a given board. To do this, you must ascertain two numbers for 
that board: the number of bombs, and the number of spaces which neither contain nor neighbor any bombs.</p>
<p>Counting the bombs is easy, but the latter sum is a bit more difficult to reach. For each space on the board, you must check each of 
...
undoubtedly easier and less error-prone to put this check into its own function:</p>
...
<p>Once you have both numbers, executing the formula is simple arithmetic - but make sure to multiply by 100, to yield a percentage.</p>
...
Used as: Division Two - Level Three: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>Dumitru</b> for 592.29 points (28 mins 1 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
<p>Your task in this problem is to determine if a given compound word is composed entirely of prefixes and/or suffixes of a given base 
...
divided.</p>
<p>Solving this problem requires knowledge of either dynamic programming or memoization, programming tricks which are rarely seen 
...
easier to code than DP.</p>
<p>To make things easier, it is wise to start your solution by creating a dictionary of valid word parts.  Simply create a new array and 
add to it every prefix and suffix of <b>original</b>, including the whole word itself.</p>

<pre>	vector&lt;string&gt; dict;
...
<p>With our dictionary in hand, we can now launch our recursive function.  This function need take only one parameter: the current 
...
including the memoization step, refer to the pseudocode below.</p>
...
<p>The memoization step is necessary for cases such as the one where both <b>original</b> and <b>compound</b> consist of 50 A's in a 
...
allowed 8 seconds.  Memoization ensures that the function will never have to do work more than 50 times.</p>
<p>One last thing to note is that the greedy algorithm fails on this problem.  Your recursive function needs to check every word in the 
...
Used as: Division One - Level Two: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>Yarin</b> for 512.45 points (7 mins 48 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
<p>The goal of this problem is to determine the optimal strategy for your company's elevator. Given the arrival floors, arrival 
...
its task.</p>
<p>The constraints are what make this problem possible. There are at most five employees to consider, which should be a small 
enough number for even the slowest of brute-force algorithms to handle in 8 seconds.</p>
<p>To understand how to account for each case, let us consider an example where the maximum five employees all need to use the 
...
and the smallest value you find will be the answer.</p>
<p>C++ users have an advantage here with the ever-useful <tt>next_permutation()</tt> function. If your language doesn't have 
...
future use.</p>
<p>Alternately, this problem can be solved by recursion.  Refer to the following pseudocode:</p>

<pre>
int timeLeft(int[] passengerState, int currentFloor) {
   ...
   for each passenger p
      ...
      otherwise, if p has not yet been picked up,
         let timeNeeded=(distance from currentFloor to p's starting floor)
            + timeLeft(new passengerState, new floor);
      otherwise, if p is currently in the elevator,
         let timeNeeded=(distance from currentFloor to p's destination floor)
         + timeLeft(new passengerState, new floor);
      if timeNeeded &lt; answer
	   set answer=timeNeeded
   return answer
}
</pre>
...
Used as: Division One - Level Three: <blockquote><table cellspacing="2"> 
  ...
  <tr> 
    ...
    <td class="bodyText" style="background: #eee;"> 
      <b>ZorbaTHut</b> for 736.40 points (14 mins 3 secs) 
    </td> 
  </tr> 
  <tr> 
    <td class="bodyText" style="background: #eee;"> 
...
<p>This problem gives you a board consisting of empty spaces, walls, and the starting positions of two players (A and B).  It then 
...
otherwise-perfect submissions, including <b>Yarin</b>'s.)</p>
<p>Yet again, the input restrictions on this problem allow a solution which would otherwise time out.  The largest possible board is 20 
...
once), it should run well within 8 seconds.</p>
<p>The best and fastest way to solve this problem is to use a breadth-first search.  The state of the board can be described as a 4-tuple: 
...
one state to another, which can be accomplished as follows:</p>
...
<p>Once we have a new state, we need to validate it.  Validation consists of several steps:</p>

<ol>
<li>Have we seen this state before?  If we've already dealt with this combination of player positions, we should skip it and go on 
...
after we've gotten past this step of the validation.</li>
<li>Are both players in valid locations? Neither player should be inside of a wall, or off the board, or in the same location.</li>
<li>Did this move cause the players to swap places?  If <tt>(playerA_newRow == playerB_row &#38;&#38; playerA_newCol == playerB_col &#38;&#38; 
playerB_newRow == playerA_row &#38;&#38; playerB_newCol == playerA_col)</tt>, then the players have crossed paths, and this move was 
invalid.</li>
<li>Did we win? If the players have switched places, we can immediately return the number of turns we've taken so far.</li></ol>

<p>After the state has been validated, we can push it (and the number of turns it took to get to it) to the back of our queue, and pop 
...
return -1.</p>
                        ...
                        <p>
                        <img src="/i/m/LunaticFringe_mug.gif" alt="" width="55" height="61" border="0" hspace="6" vspace="1" align="left" /><br />
