...
public class RegistrationHelper {
    ...
    public static void populateStudentStaticContent(StudentRegistrationBean srb)
        ...
        stateList.add(new ListPairBean(null, "Pick a state"));
        ...
        srb.setStateList(stateList);

        rsc = (ResultSetContainer) data.get("editor_list");
        List editorList = new ArrayList();
        for (Iterator i = rsc.iterator(); i.hasNext();) {
            rsr = (ResultSetContainer.ResultSetRow) i.next();
            Integer editorId = (Integer) rsr.getItem("editor_id").getResultData();
            String editorDesc = (String) rsr.getItem("editor_desc").getResultData();
            editorList.add(new ListPairBean(editorId, editorDesc));
        }

        srb.setEditorList(editorList);

        rsc = (ResultSetContainer) data.get("language_list");
        List languageList = new ArrayList();
        for (Iterator i = rsc.iterator(); i.hasNext();) {
            rsr = (ResultSetContainer.ResultSetRow) i.next();
            Integer languageId = (Integer) rsr.getItem("language_id").getResultData();
            String languageName = (String) rsr.getItem("language_name").getResultData();
            languageList.add(new ListPairBean(languageId, languageName));
        }

        srb.setLanguageList(languageList);

        List schoolList = new ArrayList();
        schoolList.add(new ListPairBean(null, "Pick a school"));
    ...
    public static void createStudent(StudentRegistrationBean srb)
        ...
        */
        ...
        try {
            ...
            User user = uh.create();
            user.createUser(userId, srb.getHandle(), NEW_USER_TYPE);
            ...
            UserSchool userSchool = ush.create();
            userSchool.createUserSchool(userId, srb.getSchoolId().longValue());
            userSchool.setCurrentUserSchoolId(userId,
            ...
            coder.createCoder(userId);
            coder.setMemberSince(userId, new java.sql.Date(new Date().getTime()));
            ...
            UserTermsOfUse userTermsOfUse = utouh.create();
            userTermsOfUse.createUserTermsOfUse(userId, TERMS_OF_USE_ID);

            RatingHome rh = (RatingHome) ctx.lookup(RatingHome.EJB_REF_NAME);
            Rating rating = rh.create();
            rating.createRating(userId);
    ...
    public static void updateStudent(StudentRegistrationBean srb)
        ...
        try {
            ...
            if (userSchool.existsUserSchoolId(userId, schoolId)) {
                userSchool.setCurrentUserSchoolId(userId, schoolId);
            }
            else {
                userSchool.createUserSchool(userId, schoolId);
                userSchool.setCurrentUserSchoolId(userId, schoolId);
            }
    ...
    public static void populateCoachFromRequest(ServletRequest _request,
        ...
        crb.setFirstName(getParameter(_request, "first_name", crb.getFirstName()));
        crb.setLastName(getParameter(_request, "last_name", crb.getLastName()));
        crb.setStateCode(getParameter(_request, "state_code", crb.getStateCode()));
        crb.setSchoolId(getParameterLong(_request, "school_id", crb.getSchoolId()));
        crb.setHandle(getParameter(_request, "handle", crb.getHandle()));
        crb.setChangePassword("true".equals(getParameter(_request,
                                                          "change_password",
                                                          "false")));
        crb.setPassword(getParameter(_request, "password", crb.getPassword()));
        crb.setConfirmPassword(getParameter(_request, "confirm_password",
                                             crb.getConfirmPassword()));
        crb.setEmail(getParameter(_request, "email", crb.getEmail()));
        crb.setConfirmEmail(getParameter(_request, "confirm_email",
                                          crb.getConfirmEmail()));
        crb.setEditorId(getParameterInteger(_request, "editor_id",
                                             crb.getEditorId()));
        crb.setLanguageId(getParameterInteger(_request, "language_id",
                                               crb.getLanguageId()));
        crb.setAgreeTerms("true".equals(getParameter(_request, "terms", "false")));
    ...
    public static void populateCoachStaticContent(CoachRegistrationBean crb)
        ...
        crb.setLanguageList(languageList);
        ...
        crb.setSchoolList(schoolList);

        TermsOfUseHome touh = (TermsOfUseHome) ctx.lookup(TermsOfUseHome.EJB_REF_NAME);
        TermsOfUse tou = touh.create();
        crb.setTermsOfUse(tou.getText(TERMS_OF_USE_ID));
    }
    ...
    public static void createCoach(CoachRegistrationBean crb)
        ...
        * one database will not force a rollback in another. This should be fixed!
        */
        UserTransaction utxCommon = null;
        UserTransaction utxTchs = null;
        ...
        try {
            ...
            utxTchs = (UserTransaction) ctx.lookup("javax.transaction.UserTransaction");
            utxTchs.begin();

            UserHome uh = (UserHome) ctx.lookup(UserHome.EJB_REF_NAME);
            User user = uh.create();
            user.createUser(userId, crb.getHandle(), NEW_USER_TYPE);
            ...
            UserSchoolHome ush = (UserSchoolHome)
                    ctx.lookup(UserSchoolHome.EJB_REF_NAME);
            UserSchool userSchool = ush.create();
            userSchool.createUserSchool(userId, crb.getSchoolId().longValue());
            userSchool.setCurrentUserSchoolId(userId,
    ...
    public static void updateCoach(CoachRegistrationBean crb) throws Exception {
        ...
        try {
            ...
            UserSchoolHome ush = (UserSchoolHome)
                    ctx.lookup(UserSchoolHome.EJB_REF_NAME);
            UserSchool userSchool = ush.create();
            long schoolId = crb.getSchoolId().longValue();
            if (userSchool.existsUserSchoolId(userId, schoolId)) {
                userSchool.setCurrentUserSchoolId(userId, schoolId);
            }
            else {
    ...
    private static boolean checkValidHandle(Map errors, String handle)
        ...
        if (handle == null || handle.length() == 0) {
            ...
            boolean technicalProblems = false;
            try {
                Context ctx = TCContext.getContext(ApplicationServer.SECURITY_CONTEXT_FACTORY,
                                                   ApplicationServer.SECURITY_PROVIDER_URL);
                PrincipalMgrRemoteHome pmrh = (PrincipalMgrRemoteHome)
