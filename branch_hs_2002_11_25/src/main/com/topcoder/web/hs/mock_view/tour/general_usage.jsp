...
<jsp:include page="/hs/includes/global/left_calendar.jsp" />
    ...
    <TR>
        <TD>
            <TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0" BGCOLOR="" WIDTH="100%">
             <TR><TD ALIGN="left" HEIGHT="44" WIDTH="348"><IMG SRC="/i/hs/headinginside_compguide.gif" WIDTH="348" HEIGHT="44" BORDER="0"></TD>
    ...
    <TR>
        ...
        <TR>
            ...
            <!--Body Starts-->
            <P><A HREF="/hs/tour/getting_started.jsp">Getting Started</A>  |  <A HREF="/hs/tour/general_usage.jsp">General TopCoder Arena Usage</A>  |  Practicing / Training in the TopCoder Arena  |  Competing in a TopCoder Rated Event  |  Further Customizing the TopCoder Arena  |  The Coding Window  |  How our software interfaces with the submissions.  |  The Room Summary Window</P>

<P><B>Logging in to the TopCoder Arena</B></P>

<P>Once the TopCoder Arena launches, you will be presented with a login screen.  From here, you need simply type in your TopCoder handle and the password that you chose during the web site registration process.  Once you log in, you will initially be placed in the main chat lobby.</P>

<P><B>General Usage and the Various Lobbies</B></P>

<P>Each room in the Arena has a chat interface to allow communication with others in the same room.  You can see who is in the room with you by checking the "Who's here" list in your room.  Each coder has a color associated with them, which is derived based on the rating/color key on the left side of the applet.  The circle next to each coder's handle represents how close they are to reaching the next rating color level.  The more full the circle, the higher their rating within their color group.  From anywhere in the Arena, you have the option to move to a different area of the Arena.  You can move to one of three types of rooms:  1) a lobby room, 2) a practice room, 3) a competition room.  A lobby room is a place to chat with others who are logged in.  There is a special lobby room called the "Admin Lobby Room" that is used to communicate with the competition administrators.  Questions about the Arena usage and the problem statements can be asked in the Admin Lobby Room.  There are numerous practice rooms that contain problem sets that have been used in past competitions.  The practice rooms are a great place to become acclimated with the Arena interface and the competition structure.  The competition rooms only become available shortly before the start of a scheduled match or tournament.  The interface for the competition rooms is very similar to that of the practice rooms.</P>
...
<P><B>Admin</B><BR/>
If at any time you have a question regarding the use of the Arena or, in the case of an active match or tournament, a question about the problems, you can use the "admins" message type.  An "admins" message type will show up on the screen of anyone in the Admin Lobby Room, regardless of where you are when you send the message.  This is convenient because you will not need to leave your competition room to query the administrators.  Typing "admins: this is my message" in the chat area, you can also send an "admins" message type.</P>

<P><B>Auto-Enhanced Chat</B><BR/>
By default, you will have access to the auto-enhanced chat functionality.  With auto-enhanced chat turned on, you will notice that when any message type is directed at a specific coder, it will default to that same message type and coder for subsequent messages.  This makes it easy to hold a conversion with another coder without typing their handle each time you would like to reply.  Auto-enhanced chat can be disabled from the Options menu at any time.  </P>

<P><B>Emoting</B><BR/>
On occasion, you may find it useful to convey an emotion in the chat area in the third person.  Any time the string "/me" appears as the first part of a chat message, the "/me" will be replaced with your TopCoder handle and the message will appear to all users in italics.  (i.e. "/me is very happy")</P>



<P><B>Other Members in the Arena</B></P>

<P><B>Getting Information about another Member</B><BR/>
