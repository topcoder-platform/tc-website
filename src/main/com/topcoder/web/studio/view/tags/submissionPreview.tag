...
<c:choose>
    <c:when test="${row.map['contest_channel_id'] eq adminV1}">
        ...
        <c:choose>
            <c:when test="${row.map['is_image']}">
                <c:set var="previewImageSrc" value="${sessionInfo.servletPath}?<%=Constants.MODULE_KEY%>=${processor}&amp;<%=Constants.SUBMISSION_ID%>=${submissionId}"/>
            </c:when>
            <c:otherwise>
    ...
    <c:otherwise>
        <%-- TC Direct and Studio Admin V2 contests --%>
            ...
            <c:if test="${not multi}">
                <c:set var="previewImageSrc" value="${sessionInfo.servletPath}studio.jpg?${modKey}=DownloadSubmission&amp;${subId}=${row.map['submission_id']}&amp;${subAltType}=medium"/>
            </c:if>
    </c:otherwise>
</c:choose>
...
<div id="sub${row.map["submission_id"]}" class="submission">
                        ...
                        <li>
                            <a class="viewFullSizeMulti" href="javascript:;">
                                <span class="prevImg${index.index}">
                                    <img src="${sessionInfo.servletPath}studio.jpg?${modKey}=DownloadSubmission&amp;${subId}=${row.map['submission_id']}&amp;${subAltType}=full&amp;${subFileIdx}=${index.index}" alt="" />
                                </span>
                            </a>
                        </li>
                    </c:forEach>
    ...
    <c:if test="${multi}">
        <div class="control">
            <a href="javascript:;" class="btnPrevious"><span class="hide">Previous</span></a>
            <a href="javascript:;" class="btnNext"><span class="hide">Next</span></a>
            <span>Image <span class="curItem">1</span> of <span class="totalCount">${row.map["gallery_image_count"]}</span></span>
        </div>
    </c:if>
    <div class="text">
		<c:if test="${showPlacement}">
    		...
    		<c:choose>
    			...
    			<c:otherwise>
		    		<c:choose>
    					<c:when test="${row.map['placed'] == 1}">1st</c:when>
    					<c:when test="${row.map['placed'] == 2}">2nd</c:when>
    					<c:when test="${row.map['placed'] == 3}">3rd</c:when>
		    			<c:otherwise>${row.map['placed']}th</c:otherwise>
    				</c:choose>
					Place
    			</c:otherwise>
    		</c:choose>
        ...
        <span class="submissionId"><strong>ID: <span class="subId">${row.map["submission_id"]}</span></strong></span><br />
		<c:if test="${viewSubmitters}">
	        <span><strong>Handle: <studio:handle coderId="${userId}"/></strong></span><br />
		</c:if>
        <span>Submitted on <fmt:formatDate value="${createDate}" pattern="MM.dd.yyyy 'at' HH:mm z" timeZone="${sessionInfo.timezone}"/></span><br />
        <c:choose>
            <c:when test="${multi}">
        ...
        <span><a class="${fullSizeClass}" href="javascript:;">View Full Size</a>&nbsp;|&nbsp;<a href="${sessionInfo.servletPath}?${modKey}=DownloadSubmission&amp;${subId}=${row.map["submission_id"]}">Download</a></span>
    </div>
</div>
