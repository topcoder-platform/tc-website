...
<ul>
    <li>入围赛
阶段，编码阶段共计60分钟，从参赛者进入第1题开始计时，共需完成2题。</li>
...
<br><br>
编码阶段允许参赛者重新提交竞赛题目方案。如果参赛者对一道题目已经交了一份方案后决定重交一份的话，那么计算这一题的得分时,时间要加上从参赛者上一次提交后过去的时间，并且还要扣除这一题的满分的10%作为额外的处罚。
<br><br>
...
<blockquote>
    ...
    参赛者在编写解决方案时所建立的类以及所有类的成员都必须定义为"public"，使TopCoder伺服器能够访问类成员，并调用适当的函数。下面可以看到Java,C++,C#,Visual Basic的代码模板：
    <br><br>
    <b>Java代码模板</b><br>
    ...
    <strong>编译</strong><br>
    编码窗口中的代码编译在TopCoder伺服器上进行。编译时编码窗口内选择的编程语言将会对相应编译器发出编译请求，同时将编码窗口内的代码发送至TopCoder编译器。如果编译成功，参赛者将被提示成功；如果编译失败，失败的错误信息将返回，并在参赛者的编译结果窗口中显示。编译成功所生成的目标文件会被存储在TopCoder的伺服器上。任何参赛者进行代码的测试或者提交都会采用最近编译成功的代码。下面是一个Java编译失败的例子：
    ...
    <br><br>
    <strong>注意：</strong>参赛者运行类或函数的任何标准输出(i.e., System.out.println)会和测试结果一同返回。这有助于参赛者调试代码。参赛者也可以在提交代码前去掉这些系统输出，以提高提交代码的运行效率。
    <br><br>
...
参赛者可以在编码的任何时候保存代码。点击"Save"按钮，当前代码就会被保存在TopCoder伺服器上。经常保存可以确保参赛者较新的代码能够备份在伺服器上，以防止意外事件的发生(诸如网络断线等情况)。编译代码的同时，代码会自动保存在伺服器上。
<br><br>
...
编码窗口的计时器会为当前竞赛倒计时，直至竞赛时间结束。例如在入围赛
阶段，计时器会提醒参赛者在1小时的比赛时间内还剩多少时间(从参赛者打开题目时开始计时)。注意在用户机与伺服器之间始终存在着延迟，因此，最好在计时器还剩至少30秒的时间内提交解决方案。
<br><br>
...
挑战阶段后，所有的提交方案都要经过一系列数据测试。例如入围赛
时，系统测试会在整个入围赛
完成后进行。数据测试会对期望值和返回值进行比较，来决定方案是否通过测试。
<br><br>
