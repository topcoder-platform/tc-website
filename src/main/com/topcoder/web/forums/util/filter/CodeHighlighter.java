...
/**
 * A Filter that syntax highlights code appearing between [code][/code] tags.
 */
public class CodeHighlighter implements Filter {
    ...
    public String applyFilter(String string, int currentIndex, FilterChain chain) {
        ...
        // short circuit this filter if no [code] found
        if (string.indexOf("[code]") < 0) {
            return chain.applyFilters(currentIndex, string);
        }

        // we have something to filter
        while ((startCodeTag = string.indexOf("[code]", endCodeTag)) >= 0) {
            int end = string.indexOf("[/code]", startCodeTag + 6);

            if (end > 0) {
                if (endCodeTag < startCodeTag) {
                    // apply filters for content between [/code] and [code]
                ...
                filtered.append(string.substring(endCodeTag, startCodeTag) + "[code]");
