...
<tr>
    ...
    <td class="rtTextCell100">
        ...
        <% if (errors.get(ForumConstants.SEARCH_QUERY) != null) { %><br/><span class="bigRed"><tc-webtag:errorIterator id="err" name="<%=ForumConstants.SEARCH_QUERY%>"><%=err%>
    ...
    <td class="rtTextCell100">
        <select size="1" name="<%=ForumConstants.SEARCH_SCOPE%>" id="<%=ForumConstants.SEARCH_SCOPE%>">
            ...
            <tc-webtag:iterator id="forum" type="com.jivesoftware.forum.Forum" iterator='<%=(Iterator)request.getAttribute("forums")%>'>
                ...
                <option value="<%=searchScopeValue%>" selected>&#149;&#160;
            ...
            <tc-webtag:iterator id="category" type="com.jivesoftware.forum.ForumCategory" iterator='<%=(Iterator)request.getAttribute("categories")%>'>
                ...
                <tc-webtag:iterator id="forum" type="com.jivesoftware.forum.Forum" iterator='<%=category.getForums()%>'>
                    ...
                    <option value="<%=forumValue%>" selected>&#149;&#160;
                        <jsp:getProperty name="forum" property="name"/>
                    </option>
                    <% } else { %>
                    <option value="<%=forumValue%>">&#149;&#160;
                        <jsp:getProperty name="forum" property="name"/>
                    </option>
                    <% } %>
                </tc-webtag:iterator>
...
<tr>
    ...
    <td class="rtTextCell100">
        <select size="1" name="<%=ForumConstants.SEARCH_DATE_RANGE%>" id="<%=ForumConstants.SEARCH_DATE_RANGE%>">
            ...
            <option value="<%=ForumConstants.SEARCH_DATES[i]%>" selected>&#149;&#160;<%=
                ForumConstants.SEARCH_DATE_LABELS[i]%> (<%=
                formatter.format((Date) dates.get(ForumConstants.SEARCH_DATES[i]))%>)
            </option>
            <% } else { %>
            <option value="<%=ForumConstants.SEARCH_DATES[i]%>">&#149;&#160;<%=ForumConstants.SEARCH_DATE_LABELS[i]%>
                (<%=formatter.format((Date) dates.get(ForumConstants.SEARCH_DATES[i]))%>)
            </option>
            <% } %>
            <% } %>
...
<tr>
    ...
    <td class="rtTextCell100">
        ...
        <% if (errors.get(ForumConstants.SEARCH_HANDLE) != null) { %><br/><span class="bigRed"><tc-webtag:errorIterator id="err" name="<%=ForumConstants.SEARCH_HANDLE%>"><%=err%>
    </tc-webtag:errorIterator></span><% } %>
    </td>
</tr>
<tr>
    ...
    <td class="rtTextCell100">
        <select size="1" name="<%=ForumConstants.SEARCH_RESULT_SIZE%>" id="<%=ForumConstants.SEARCH_RESULT_SIZE%>">
            ...
            <option value="<%=resultSizes[i]%>" selected><%=resultSizes[i]%>
            </option>
            <% } else { %>
            <option value="<%=resultSizes[i]%>"><%=resultSizes[i]%>
            </option>
            <% }
            } %>
        </select>
