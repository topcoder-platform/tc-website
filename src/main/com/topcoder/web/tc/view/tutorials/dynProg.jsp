...
<!-- Center Column Begins -->
         ...
         <div class="tutorBodyFull">
			...
			By&#160;<a class="bodyText" href="/stat?c=member_profile&amp;cr=7305703"><strong>Dumitru</strong></a><br />
...
<br/><br/>
<span class=bodySubtitle>Introduction (Beginner)</span><br/>
<em>What is a dynamic programming, how can it be described?</em>
<br/><br/>
A <b>DP</b> is an algorithmic technique which is usually based on a recurrent formula and one (or some) starting <span class=bigRed>states</span>. A sub-solution of the problem is constructed from previously found ones. 
...
<br/><br/>
First of all we need to find a <span class=bigRed>state</span> for which an optimal solution is found and with the help of which we can find the optimal solution for the next <span class=bigRed>state</span>.
<br/><br/>
<em>What does a "<span class=bigRed>state</span>" stand for?</em>
<br/><br/>
It's a way to describe a situation, a sub-solution for the problem. For example a <span class=bigRed>state</span> would be the solution for sum <b>i</b>, where <b>i&#8804;S</b>. A smaller <span class=bigRed>state</span> than state <b>i</b> would be the solution for any sum <b>j</b>, where <b>j&lt ;i</b>. For finding a <b>state i</b>, we need first to find all smaller states <b>j (j&lt;i) </b>. Having found the minimum number of coins which sum up to <b>i</b>, we can easily find the next state - the solution for <b>i+1</b>.
<br/><br/>
<em>How can we find it?</em>
<br/><br/>
...
First of all we mark that for <span class=bigRed>state 0</span> (sum 0) we have found a solution with minimum number of 0 coins. 
...
Then we proceed to the next <span class=bigRed>state</span> - <b>sum 2</b>. We again see that the only coin which is less or equal to this sum is the first coin, having a value of 1. The optimal solution found for sum (2-1) = 1  is coin 1. This coin 1 plus the first coin will sum up to 2, and thus make a sum of 2 with the help of only 2 coins. This is the best and only solution for sum 2.
...
The same we do for sum 4, and get a solution of 2 coins - 1+3.
And so on.
...
Now let's see how to find a way for passing from one <span class=bigRed>state</span> to another, for harder problems. For that we will introduce a new term called <span class=bigRed>recurrent relation</span>, which makes a connection between a lower and a greater <span class=bigRed>state</span>.
...
First of all we have to find a <span class=bigRed>state</span>.
...
At each step we find the unmarked <span class=bigRed>state</span> (i,j) for which the shortest path was found. We mark it as visited (not to use it later), and for each of its neighbors we look if the shortest path to it may be improved. If so - then update it.
We repeat this step until there will remain no unmarked <span class=bigRed>state</span> to which a path was found. The solution will be represented by <b>Min[N-1][j] </b> having the least value (and the greatest <b>j</b> possible among the <span class=bigRed>states</span> having the same value, i.e. the shortest paths to which has the same length).
...
Given a matrix with <b>M</b> rows and <b>N</b> columns (<b>N x M</b>). In each cell there's a number of apples.
...
When have read the description of a problem and started to solve it, first look at its restrictions. If a polynomial-time algorithm should be developed, then it's possible that the solution may be of <span class=bigRed>DP</span> type. In this case try to see if there exist such <span class=bigRed>states</span> (sub-solutions) with the help of which the next <span class=bigRed>states</span> (sub-solutions) may be found. Having found that - think about how to pass from one <span class=bigRed>state</span> to another. If it seems to be a <span class=bigRed>DP</span> problem, but you can't define such <span class=bigRed>states</span>, then try to reduce the problem to another one (like in the example above, from Section 5).
