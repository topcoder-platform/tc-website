...
if (roundId == 17428) {
  response.sendRedirect("https://www.topcoder.com/challenges/30085346");
  return;
}

	
if (roundId == 17378) {
  response.sendRedirect("https://www.topcoder.com/challenges/30076905");
  return;
}
%>
            ...
            <tbody>
                <tr>
                    <td style="padding: 0px 5px 10px 0px;" align="center" width="50%">
                        ...
                        <a style="width: 130px; font-weight:bold;" href="/longcontest/?module=ViewProblemStatement&amp;<%=Constants.ROUND_ID%>=${row.map['round_id']}&amp;pm=${row.map['problem_id']}">Problem Statement</a>
...
<% if (roundId == 16752) { %>
<div class="sectionHeader">Event Preview</div>
<p><strong>This is not an actual contest, but may provide a useful preview for
upcoming contests.</strong></p>

<p>On Friday, June 10th, Topcoder will record an informational webinar with the
team from Harvard and the Broad Institute to discuss ‘Connectivity Map’ and the
upcoming precision medicine
<a href="http://crowdsourcing.topcoder.com/cmap">Topcoder Marathon Match CMAP1</a>
(Connectivity Map challenge #1). The purpose of this webinar is to provide interested parties with
a deeper understanding of precision medicine, CMAP, how it is used, why it
matters, and technically, how does it work. This webinar will particularly be
beneficial for those looking to compete CMAP challenges.</p>

<p>Note: The recording will be made publicly available after the webinar.</p>
<% } %>

<% if (roundId == 16753) { %>
<div class="sectionHeader">Problem Preview</div>

<p>The Connectivity Map (CMap) is a Broad Institute project that enables the
discovery of functional connections between drugs, genes and diseases through
the generation and analysis of gene expression signatures. Recent technological
advances and laboratory automation have enabled data generation scale-up
resulting in a massive catalog of drug and gene signatures. When coupled with
powerful pattern recognition algorithms, these data represent a rich reference
library for the research community.</p>

<p>CMap utilizes a novel, high-throughput gene expression profiling technology
to generate gene expression profiles at scale. The crux of this approach is that
instead of measuring all ~20,000 genes in the human genome, CMap measures a
select subset of approximately 1,000 genes and uses these “landmark” gene
measurements to computationally infer a large portion of the remainder. The
current algorithm is effective but imperfect, and improving the imputation
methods will have an immediate impact on the quality of data and the
biologically meaningful connections that can be discovered. With this in mind,
we have designed our first contest to stimulate the exploration of new and
improved inference methods.</p>

<p><a href="http://crowdsourcing.topcoder.com/cmap">Pre-register for this exciting challenge today</a>
and you'll receive timely updates so you don't miss out on any important details!</p>
<% } %>
...
<% if (roundId == 16641) { %>
<div class="sectionHeader">Problem Preview</div>
<p>Santa has to deliver sweets around the world, but regions around the world have storms.
The sweets can go bad at different rates or get delivered at different efficiency depending
on the type of sweet. The goal is to ensure the best behaved children get the most of the
sweets. What should Santa deliver, and where, to ensure objective is attained?</p>
<% } %>

<% if (roundId == 16638) { %>
<div class="sectionHeader">Problem Preview</div>
<p>The genomes of cancer cells are often very different from those of healthy cells.
One common change in tumor cells is the loss or gain of specific pieces of DNA.
The fraction of genome altered, or FGA, is a measure of how much the DNA of tumor cells has changed.
The FGA of prostate cancer tissue is a predictor of survival.</p>

<p>The objective of this marathon match is to create an algorithm that uses images of cancerous
prostate cancer biopsies to predict the FGA values of those tissues.</p>
<% } %>

<% if (roundId == 16672) { %>
<div class="sectionHeader">Problem Preview</div>
<p>Robonaut 2 (“R2”), a humanoid robot that operates both on Earth and on the International Space
Station, commonly uses tools. For example, it manages inventory using an RFID reader and fastens
bolts with a drill. In order to use a tool, R2 relies on an algorithm to determine a 3D
representation of the tool. The algorithm works with the robot’s control system and allows R2 to
create a plan for grasping objects and completing its tasks.</p>

<p>There exist several algorithms that could be used to determine the 3D representation of the
tool. However, the robot employs an older, less capable set of vision sensors, due to its space
heritage and having been exposed to high levels of environmental radiation over time. Many existing
algorithms assume that the vision data being used is of relatively high resolution, detail, and
quality, and such algorithms are not effective when used with the grade of vision data available to
R2. As a result, the R2 team needs you to create vision algorithms for determining the 3D
representations of 6 different tools that will be effective with noisy, stereo vision data.</p>

<p>The objective here is to create algorithms that will receive a pair of noisy stereo images of
common space tools such as an RFID reader, an EVA handrail, or a softbox, among others, and
determine the 3D representation of the object in the image pair.</p>
<% } %>


<% if (roundId == 16676) { %>
<div class="sectionHeader">Problem Preview</div>
<p>A media company generates revenue by selling advertising space to firms in which to advertise
their products and services. Advertisers pay for access to audiences of certain kinds of media
consumers, and an important demographic into which some consumers fall is called Demographic X.
Being able to demonstrate that Demographic X media consumers constitute a large proportion of
those who consume their media is important for the company as they price and sell advertising
space. The objective of this match is to create an algorithm that uses demographic data on
individual media consumers to predict whether or not they are in Demographic X.</p>

<p>This match will have $20,000 in prizes available, we hope you'll join us and participate in
solving this problem.</p>
<% } %>

<% if (roundId == 16677) { %>
<div class="sectionHeader">Problem Preview</div>
<p>An electronics parts supplier sells thousands of parts to retailers nationwide. The firm’s
customers view a certain subset of parts differently from the rest. As a result, the firm refers to
these items as “special parts” which designates them as frequently purchased parts regardless of
market price over time. The objective of this challenge is to create an algorithm that uses
independent variables to predict which parts are considered the suppliers "special parts".</p>
<% } %>
