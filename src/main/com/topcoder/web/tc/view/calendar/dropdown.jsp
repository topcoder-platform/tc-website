...
<form><select name="month" size="1" onchange="window.location='/tc?module=Static&amp;d1=calendar&amp;d2='+this.options[this.selectedIndex].value">
   ...
   <option value="jan_12"<% if (currMonth.equals("jan_12")) { %> selected<% } %>>January</option>
   <option value="feb_12"<% if (currMonth.equals("feb_12")) { %> selected<% } %>>February</option>
   <option value="mar_12"<% if (currMonth.equals("mar_12")) { %> selected<% } %>>March</option>
   <option value="apr_12"<% if (currMonth.equals("apr_12")) { %> selected<% } %>>April</option>
