...
<td class="divider">
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="statTable">
        <tr>
            <td class="tableTitle" colspan="2">Development Competitions</td>
        </tr>
        <tr>
            <td class="ratingBox" colspan="2">Rating:<br>
                ...
                <tc-webtags:ratingColor rating='<%=rscDevData.getIntItem(0, "rating")%>'>
                    <rsc:item name="rating" set="<%=rscDevData%>" format="####"/>
        ...
        <tr>
            <td class="catNW" colspan="2">
                ...
                <A href='/tc?module=OutstandingProjects&<%=Constants.PHASE_ID%>=113&cr=<%=request.getParameter("cr")%>'>[current
                    contests]</A><br>
                <A href="/tc?module=ReliabilityDetail&ph=113&uid=<%=request.getParameter("cr")%>">[reliability
                    detail]</A><br><br>
            </td>
        </tr>
        <tr>
            ...
            <td class="statRNW">
                <rsc:item name="percentile" set="<%=rscDevData%>" ifNull="N/A"/>
            </td>
        </tr>
        <tr>
            <td class="catNW">Rank:</td>
            <td class="statRNW">
                <rsc:item name="rank" set="<%=rscDevData%>" ifNull="not ranked"/>
                <% if (rscDevData.getStringItem(0, "rank") != null) { %> of
                <rsc:item name="num_ranked" set="<%=rscDevData%>"/>
                <% } %></td>
        </tr>
        <tr>
            <td class="catNW">Country Rank:</td>
            <td class="statRNW">
                <rsc:item name="country_rank" set="<%=rscDevData%>" ifNull="not ranked"/>
                <% if (rscDevData.getStringItem(0, "country_rank") != null) { %> of
                <rsc:item name="num_country_ranked" set="<%=rscDevData%>"/>
                <% } %></td>
        </tr>
        ...
        <tr>
            ...
            <td class="statRNW">
                <rsc:item name="school_rank" set="<%=rscDevData%>" ifNull="not ranked"/>
                <% if (rscDevData.getStringItem(0, "school_rank") != null) { %> of
                <rsc:item name="num_school_ranked" set="<%=rscDevData%>"/>
                <% } %></td>
        </tr>
        <% }%>
        <tr>
            ...
            <td class="statRNW">
                <rsc:item name="vol" set="<%=rscDevData%>" format="####"/>
            </td>
        </tr>
        <tr>
            ...
            <td class="statRNW">
                <A href='/tc?module=CompetitionHistory&<%=Constants.PHASE_ID%>=113&cr=<%=request.getParameter("cr")%>'>
                    <rsc:item name="num_ratings" set="<%=rscDevData%>" format="####"/>
                </A></td>
        </tr>
        <tr>
            <td class="catNW">Reliability:</td>
            <td class="statRNW"><A href="/tc?module=ReliabilityDetail&ph=113&uid=<%=request.getParameter("cr")%>">
                <rsc:item name="reliability" set="<%=rscDevData%>" format="#.##%" ifNull="n/a"/>
            </A></td>
        </tr>
        <tr>
            <td class="catNW">Maximum Rating:</td>
            <td class="statRNW">
                <rsc:item name="max_rating" set="<%=rscDevData%>" format="####"/>
            </td>
        </tr>
        <tr>
            <td class="catNW">Minimum Rating:</td>
            <td class="statRNW">
                <rsc:item name="min_rating" set="<%=rscDevData%>" format="####"/>
            </td>
        </tr>
    </table>
</td>
    ...
    <td colspan="2" align="center" style="border-top: 1px solid #999999;">
        <script language="JavaScript">
            var s = '<object ' +
                    ...
                    '<param name="movie" value="/flash/graphLoader20090420.swf?competitionType=dev&coderID=<%=request.getParameter("cr")%>&myRating=<%=rscDevData.getIntItem(0, "rating")%>" />' +
                    '<param name="menu" value="true" /> ' +
                    '<param name="quality" value="high" /> ' +
                    '<param name="bgcolor" value="#EEEEEE" /> ' +
                    '<embed ' +
                    'src="/flash/graphLoader20090420.swf?competitionType=dev&coderID=<%=request.getParameter("cr")%>&myRating=<%=rscDevData.getIntItem(0, "rating")%>" ' +
                    'menu="true" ' +
                    'quality="high" ' +
                    'bgcolor="#EEEEEE" ' +
                    'width="600" ' +
