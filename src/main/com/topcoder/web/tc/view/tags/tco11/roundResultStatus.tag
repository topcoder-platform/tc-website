    ...
    <c:when test="${not empty roundResult}">
        <c:choose>
            <c:when test="${fn:trim(roundResult) eq 'Champion'}">
                <img src="/i/tournament/tco11/prize.gif"/>
            </c:when>
            <c:when test="${index mod 2 == 0}">
                <img src="/i/tournament/tco11/${fn:trim(fn:toLowerCase(roundResult))}_odd_ico.png" alt="${roundResult}"/>
            </c:when>
            <c:otherwise>
                <img src="/i/tournament/tco11/${fn:trim(fn:toLowerCase(roundResult))}_even_ico.png" alt="${roundResult}"/>
            </c:otherwise>
        </c:choose>
    </c:when>
    <c:when test="${empty prevRoundResult or (prevRoundResult eq 'Eliminated')}">&nbsp;</c:when>
    <c:otherwise>
        <img src="/i/tournament/tco11/not_compete_ico.png" alt="Did not compete"/>
    </c:otherwise>
</c:choose>
