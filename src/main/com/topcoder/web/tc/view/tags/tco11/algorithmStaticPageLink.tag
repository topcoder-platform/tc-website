...
<c:set var="url"
       value="/tc?${requestScope.MODULE_KEY}=Static&amp;d1=tournaments&amp;d2=tco11&amp;d3=algorithm&amp;d4=${page}"/>

<c:choose>
    <c:when test="${page eq 'leaderboard'}">
        <c:set var="url" value="/tco11?${requestScope.MODULE_KEY}=AlgorithmAdvancers"/>
    </c:when>
    <c:when test="${page eq 'qualification'}">
        <c:set var="url" value="/tco11?${requestScope.MODULE_KEY}=AlgorithmQualification"/>
    </c:when>
</c:choose>
