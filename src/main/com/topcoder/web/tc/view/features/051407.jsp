...
<p>One nice trick you can use if you just want to create a copy of a table <strong>without</strong> copying the records is to just supply a WHERE CLAUSE of "where 0=1".  Note that when you use the CREATE TABLE AS command, the foreign key constraints, indexes, etc. are <strong>not</strong> copied from the original table. </p>
...
<p>Note that we using the OracleCallableStatement class which allows us to call our stored procedure.  The question marks (i.e. the ? characters) in the tc_maint_pkg.insert_coder_rec call are parameter placeholders, more commonly referred to as "bind variables".  We then specify the parameter values for each parameter of the procedure and call the OracleCallableStatement.execute() method to run the procedure.  After we run this code, if we query our coder table for these two records we'll see that they were successfully inserted:</p>
