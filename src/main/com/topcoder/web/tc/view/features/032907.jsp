...
<span class="bigTitle">A Crash Course in Relational Databases, Part II</span>
<br /><br />
...
<p>With the information in the first part of this article, you should have been able to get your own Oracle Express Edition database up and running, and get it updated with TopCoder's active algorithm competitor data. In Part II, we'll look at some of the key concepts and terms that you will encounter as you try to manage and maintain a database like this.</p>
...
<p>Not only do we get the performance benefit of this index when looking up records by coder handle, but another thing the index does for us is enforce uniqueness on the coder handle.   In other words, if we try to insert another record with the same coder handle we will get an error message.  Note that even though the handle is unique for every record, it is not part of the primary key.  We already have a primary key on coder_id.</p>
...
<p>The optimizer is a huge subject in itself, but for now just remember that it's the engine that figures out if it's going to use your indexes or not.  Sometimes the optimizer will decide that it is best <b>not</b> to use your index, and most of the time the optimizer is right.  There are a lot of things that the optimizer can do, such as breaking your query up into smaller pieces that can be run in parallel, creating temporary indexes on the fly, and many other powerful techniques that are sometimes much faster than using the index you created.  Most databases have utilities which will allow you to see the optimizer's query plan so you can determine exactly what indexes are going to be used.</p>

<p>In addition, some databases such as Oracle have "hints" you can provide to the optimizer if you want to override the plan and force the use of an index.  Depending on the database vendor, the optimizer can choose to override your hint if it decides to.  Typically, you do not want to get in the habit of using hints because they generally mean you have bigger problems, but there are times when hints can come in very handy.</p>
...
<p>If you have worked in different software shops you will discover that there are several schools of thought on referential integrity.  In a typical shop, you will have folks called Database Administrators (more commonly known as DBAs) who are responsible for creating objects in the database, taking backups, tuning the database in some cases and many other critical tasks.  While many DBAs are strong proponents of referential integrity, others look at it as a performance bottleneck and they avoid it like the plague.  Their philosophy is that the "applications" should handle ensuring the integrity of the data.  The key thing to remember here is that if you cannot guarantee the integrity of your data, it doesn't matter how fast you can retrieve it from the database! </p>
<p>Using application code to enforce referential integrity is a very dangerous practice -- what if you want to write a new application next year to run against that database?  What if a new programming language comes along (that's never happened, has it?) and you want to port your application to this new language?  If you have built all your referential integrity logic into application code, you have to carefully ensure that you port it to the new language.  But if all your referential integrity rules are built into the database with foreign keys, then all the logic is in one place.  You can then use that latest new language to re-write your application without any worries about whether the referential integrity rules remain intact.  Moreover, you can write any additional applications to your heart's content and never have to recode any referential integrity rules.</p>
<p>As for me, you can probably guess that I am a very strong proponent of putting referential integrity in the database rather than writing application code to enforce it.  I cannot count the number of times that I have had to write "cleanup" SQL scripts because data in the database has become invalid due to lack of proper foreign key relationships between tables. -- and believe me, these scripts can be very difficult to write, because most of the time you are missing records that got deleted when they really shouldn't have.</p>
<p>Consider our example above, where a record gets deleted from our coder table accidentally, but there is a round_history record associated with it.  You now have an "orphaned" round_history record and no way to get the coder information back (other than reloading the RSS feed).  But wait, the application code was supposed to prevent that from happening!  Well, apparently there was a bug, and as a result we are now missing a record.  While this may be a trivial example, consider a more extreme example where an invoice record gets deleted from a sales database because the IT group decided using referential integrity was too much trouble.  In my experience,  DBAs who cut corners in the area of referential integrity  will sooner or later end up paying for it, in a major way.  .  If you have any say in the decision making process of your company's database design, I would strongly encourage you to use referential integrity.  It will pay huge dividends for you, your successors, and your company in ensuring the integrity of your company's data. </p>
...
One of the most important features of a relational database is the <b>transaction</b>.  Put simply, a transaction is a unit of work done in the database to accomplish a task.  Thomas Kyte, in his book Expert Oracle, defines a transaction as "taking the database from one consistent state to the next."  A transaction may consist of a single update statement, or hundreds of statements, depending on the complexity.  The key thing to remember about transactions is that either <b>all</b> the work is done in a transaction, or <b>none</b> of the work is done.</p>
<p>Think about going to the Automatic Teller Machine at a bank and transferring some money from your checking account to your savings account, for example.  There are clearly multiple steps involved in this process: subtracting the transfer amount from the source account, and adding the amount to the target account.  However, if anything goes wrong in any step, we don't want any of the accounts to be updated.  Imagine what would happen if the amount was subtracted from your source account but something went wrong and the amount didn't get added to your target account.    With a transaction, if anything goes wrong all the changes are "rolled back," meaning the database is put back into the state it was in before the transaction started.  It's as if the transaction never happened.</p>
<p>As we mentioned earlier, the way we tell the database we are finished with our transaction is with the 'commit' command.  This command makes our changes permanent in the database.  If we run into a problem (insufficient funds, for example, in a banking transaction), we can issue a 'rollback' command that will effectively 'undo' the changes we made to the database.</p>

<p><span class="bodySubtitle">XI. Triggers</span><br />
Database <b>triggers</b> provide a mechanism for having a piece of PL/SQL code execute when some action occurs on a table, such as a record insert or update.  One common use of triggers is customized auditing on a table.  Oracle has many auditing capabilities available, though, so it's usually best not to reinvent the wheel.</p>
<p>Consider a SKU (short for "stock keeping unit") table where we store product information on the products our company sells.  Assume also that we have a SKU_COST table, which contains the cost for each SKU.  Since the cost for each SKU can change periodically, we would like to keep track of the costs of an SKU over time.  One way to accomplish this tracking would be to create a database trigger on our SKU table, which would fire each time an SKU record was inserted or updated.  On each insert, the trigger would fire to insert a record into the SKU_COST table, since an insert would represent a new SKU.  However, on updates we would only insert a record into the SKU_COST table if the cost for the SKU changed.  We could also keep effective_date and expiration_date columns in our SKU_COST table so we could determine what things cost at different times.  The trigger could handle updating the dates accordingly.</p>
<p>There are many other different ways to use triggers.  The thing to remember, though, is that triggers involve work for the database since extra SQL code has to be executed during each row insert, update, and so forth (depending of course on which events fire the trigger).  If you have a choice between using a trigger or a check constraint, it's usually better to use a check constraint since that involves less overhead than firing a trigger.</p>

<p><span class="bodySubtitle">XII. Conclusion</span><br />
We've covered a lot of ground in these articles.  Hopefully you know a little more about relational databases than you did before you read this article, and you can better appreciate the value  of storing and querying your data in a relational database.  You should also have a better understanding of the importance of enforcing data integrity, through tools like referential integrity, to ensure that your company's data is accurate and reliable. Or, at the least, learning how to store RSS feeds in a database should give you a novel to interact with the Web.</p>
...
<p><b>References</b><br>
<a target="_blank" href="<tc-webtag:linkTracking link='http://asktom.oracle.com/pls/asktom/f?p=100:11:0::::P11_QUESTION_ID:27523665852829' refer='feature_032907' />">Expert Oracle</a> - Thomas Kyte 
</p>
