...
<!-- Center Column Begins -->
         ...
         <div class="tutorBodyFull">
			...
			By&#160;<a class="bodyText" href="/stat?c=member_profile&amp;cr=7465390"><strong>Dan[Popovici]</strong></a><br />
...
There are several regular expression parsing libraries available for C++, unfortunately they are not very compatible with each other. Fortunately as a topcoder in the arena one does not have to cope with all this variety of "not so compatible with one another" libraries. 
...
Here is a short description of both of these APIs, leaving up to the coder to choose the one that he likes the most.


POSIX_regex
-----------

Includes support for two different regular expression syntaxes, basic and extended. Basic regular expressions are similar to those in ed, while extended regular expressions are more like those in egrep, adding the '|', '+' and '?' operators and not requiring backslashes on parenthesized subexpressions or curly-bracketed bounds. Basic is the default, but extended is preffered.

With POSIX, you can only search for a given regular expression; you can't match it.  To do this, you must first compile it in a pattern buffer, using `regcomp'. Once you have compiled the regular expression into a pattern buffer you can search in a null terminated string using 'regexec'. If either of the 'regcomp' or 'regexec' function fail they return an error code. To get an error string coresponding to these codes you must use 'regerror'. To free the allocated fields of a pattern buffer you must use 'regfree'.

For an in depth description of how to use these functions please consult [2] or [3].
...
Here is a small piece of code showing how these functions can be used:
	...
	if (regexec(&reg,str.c_str(),1,matches,0)==0) {
		cout &lt;&lt; "Match " &lt;&lt; str.substr(matches[0].rm_so,matches[0].rm_eo-matches[0].rm_so) &lt;&lt; " found starting at: " &lt;&lt; matches[0].rm_so &lt;&lt; " and ending at " &lt;&lt; matches[0].rm_eo &lt;&lt; endl;
	} else {
...
The GNU_regex API has a richer set of functions. With GNU regex function you can both match a string with a pattern and search a pattern in a string. The usage of these functions is somehow similar with the usage of the POSIX functions: a pattern must be first compiled with 're_compile_pattern', and the pattern buffer obtained is used to search and match. The functions used for searching and matching are 're_search' and 're_match'. 
In case of searching a fastmap can be used in order to optimize search. Without a fastmap the search algorithm tries to match the pattern at
consecutive positions in the string. The fastmap tells the algorithm what are the characters from which a match can start. The fastmap is constructed by calling the 're_compile_fastmap'. The GNU_regex also provides the functions 're_search2' and 're_match2' for searching and matching with split data. To free the allocated fields of a pattern buffer you must use 'regfree'.
...
Example:
	...
	if (re_search(&buffer,str.c_str(),str.size(),0,str.size(),&regs)!=-1) {
		cout &lt;&lt; "Match " &lt;&lt; str.substr(regs.start[0],regs.end[0]-regs.start[0]) &lt;&lt; " found starting at: " &lt;&lt; regs.start[0] &lt;&lt; " and ending at " &lt;&lt; regs.end[0] &lt;&lt; endl;
	} else {
		cout &lt;&lt; "Match not found." &lt;&lt; endl;
	}
	regfree(&buffer);	
...
The folowing examples are written all in Java for the sake of clarity. A C++ user can use the POSIX or the GNU regex APIs to construct functions similar to those available in Java(replace_all, split, matches).
