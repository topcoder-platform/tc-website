...
public class PayORProjects extends DBUtility {
    ...
    private static final String SQL_QUERY_PROJECTS = "SELECT p.project_id, pcl.project_category_id, pcl.project_type_id, pi56.value::int as mm_round_id,"
            +
            " (CASE WHEN " +
    ...
    private static final String SQL_QUERY_PAYMENTS = "SELECT pp.project_payment_id," +
    ...
    private static final String SQL_QUERY_PENDING_USERS = "SELECT distinct ri.value::int as user_id " +
    ...
    private static final String SQL_QUERY_PENALTIES = "SELECT ri.value::int as user_id, nvl(sum(ld.delay),0) as total_delay,"
            +
     ...
     * The method first retrieves the list of projects that have pending payments
     * and then processes each project separately.
     * 
    ...
    protected void runUtility() throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                while (rs2.next()) {
                    ...
                    if (pendingUserIds.contains(userId)) {
                        log.info("Payment " + projectPaymentId + " for user " + userId
                                + " is skipped because the user still has pending late deliverables.");
                        continue;
                    }

                    // Skip copilot payment if the review feedback hasn't been submitted yet (if
                    // applicable for this contest).
                    ...
                    // If the payment type is "mergeable" it means all user payments of that type
                    ...
                    if (rs2.getBoolean("mergeable")) {
                        ...
                        for (Payment payment2 : payments) {
                            if (payment2.userId == payment.userId
                                    && payment2.pactsPaymentTypeId == payment.pactsPaymentTypeId) {
                                refPayment = payment2;
                                break;
                            }
                        }
                ...
                // by transferring them to PACTS and updating the records in project_payment
                // table.
                for (Payment payment : payments) {
                    ...
                    // Get the penalty percentage. Copilot payments and payments for Marathon
                    // Matches are not subject to penalties.
                    ...
                    if (payment.pactsPaymentTypeId == BasePayment.CONTEST_PAYMENT) {
                        if (projectCategoryId == MARATHON_MATCH_PROJECT_CATEGORY_ID) { // If Marathon Match
                            ...
                            pactsPayment = new MarathonMatchNonTaxablePayment(payment.userId, amount, mmRoundId,
                                    payment.place);
                        } else {
                            ...
                            if (payment.place == 1 && projectTypeId != 3) {
                                ...
                                if (String.valueOf(amount).indexOf('.') != -1) {
                                    if (String.valueOf(amount).length() - String.valueOf(amount).indexOf('.') == 2) {
                                        amountInCents = Long.parseLong((String.valueOf(amount) + "0").replace(".", ""));
                                    } else if (String.valueOf(amount).length()
                                            - String.valueOf(amount).indexOf('.') == 1) {
                                        amountInCents = Long
                                                .parseLong((String.valueOf(amount) + "00").replace(".", ""));
                                    } else {
                                        amountInCents = Long.parseLong(String.valueOf(amount).replace(".", ""));
                                    }
                                } else {
                                    amountInCents = Long.parseLong(String.valueOf(amount) + "00");
                                }
                        ...
                        pactsPayment = BasePayment.createPayment(payment.pactsPaymentTypeId, payment.userId, amount,
                                projectId, payment.place);
                    }

                    log.info("Adding PACTS payment: user " + pactsPayment.getCoderId() + ", amount "
                            + pactsPayment.getGrossAmount() +
                    ...
                    if (pactsPayment2 != null) {
                        log.info("Adding PACTS payment: user " + pactsPayment2.getCoderId() + ", amount "
                                + pactsPayment2.getGrossAmount() +
            ...
            log.info("-----------------------------------------------");
            log.info("Successfully analyzed " + projectsAnalyzed + " projects and paid " + paidProjectPayments
                    + " project payments.\n");
        } catch (Exception e) {
    ...
    /**
     * Sets the project_payment.pacts_payment_id field, which effectively marks the
     * project payments as paid.
     *
    ...
    private void sendFailureEmail(Exception cause) throws Exception {
        ...
        String emailBody = "An error happened when processing Online Review payments.\n" +
