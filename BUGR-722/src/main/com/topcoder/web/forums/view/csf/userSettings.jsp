...
<tc-webtag:errorIterator id="errSettings" name="<%=ForumConstants.STATUS%>"><%=errSettings%>
...
<table cellpadding="0" cellspacing="0" border="0" class="tabTable" style="clear:both;">
    <tr>
        <td width="20%">
            <A id="tabGen" href="javascript:void(0)" onClick="toggleTabs(this,'gen')" class="tabLinkOff"><span>General Settings</span></A>
        </td>
        <td width="20%">
            <A id="tabRate" href="javascript:void(0)" onClick="toggleTabs(this,'rate')" class="tabLinkOff"><span>Rating Preferences</span></A>
        </td>
        <td width="40%"></td>
    </tr>
</table>
...
<tr style="display:none;" id="bodyGen2">
    ...
    <td class="rtThreadCell" style="width:100%;">
        <select size="1" name="forumsPerPage">
            ...
            <option value="<%=forumCounts[i]%>" selected><%=forumCounts[i]%>
            </option>
            <% } else { %>
            <option value="<%=forumCounts[i]%>"><%=forumCounts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen3">
    ...
    <td class="rtThreadCell">
        <select size="1" name="threadsPerPage">
            ...
            <option value="<%=threadCounts[i]%>" selected><%=threadCounts[i]%>
            </option>
            <% } else { %>
            <option value="<%=threadCounts[i]%>"><%=threadCounts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen4">
    ...
    <td class="rtThreadCell">
        <select size="1" name="messagesPerPage">
            ...
            <option value="<%=messageCounts[i]%>" selected><%=messageCounts[i]%>
            </option>
            <% } else { %>
            <option value="<%=messageCounts[i]%>"><%=messageCounts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen5">
    ...
    <td class="rtThreadCell">
        <select size="1" name="messagesPerHistoryPage">
            ...
            <option value="<%=historyCounts[i]%>" selected><%=historyCounts[i]%>
            </option>
            <% } else { %>
            <option value="<%=historyCounts[i]%>"><%=historyCounts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen6">
    ...
    <td class="rtThreadCell">
        <select size="1" name="resultsPerSearchPage">
            ...
            <option value="<%=searchCounts[i]%>" selected><%=searchCounts[i]%>
            </option>
            <% } else { %>
            <option value="<%=searchCounts[i]%>"><%=searchCounts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen7">
    ...
    <td class="rtThreadCell">
        <select size="1" name="threadMode">
            ...
            <option value="<%=threadModes[i][0]%>" selected><%=threadModes[i][1]%>
            </option>
            <% } else { %>
            <option value="<%=threadModes[i][0]%>"><%=threadModes[i][1]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyGen8">
    ...
    <td class="rtThreadCell">
        <select size="1" name="flatMode">
            ...
            <option value="<%=flatModes[i][0]%>" selected><%=flatModes[i][1]%>
            </option>
            <% } else { %>
            <option value="<%=flatModes[i][0]%>"><%=flatModes[i][1]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyRate3">
    ...
    <td class="rtThreadCell" valign="top">
        <select size="1" name="ratingHighlightThreshold">
            ...
            <option value="<%=ratingHltPcts[i]%>" selected><%=ratingHltPcts[i]%>
            </option>
            <% } else { %>
            <option value="<%=ratingHltPcts[i]%>"><%=ratingHltPcts[i]%>
            </option>
            <% }
            } %>
        </select>
        ...
        <select size="1" name="ratingHighlightMinCount">
            ...
            <option value="<%=ratingHltCnts[i]%>" selected><%=ratingHltCnts[i]%>
            </option>
            <% } else { %>
            <option value="<%=ratingHltCnts[i]%>"><%=ratingHltCnts[i]%>
            </option>
            <% }
            } %>
        </select>
...
<tr style="display:none;" id="bodyRate4">
    ...
    <td class="rtThreadCell" valign="top">
        <select size="1" name="ratingCollapseThreshold">
            ...
            <option value="<%=ratingCollapsePcts[i]%>" selected><%=ratingCollapsePcts[i]%>
            </option>
            <% } else { %>
            <option value="<%=ratingCollapsePcts[i]%>"><%=ratingCollapsePcts[i]%>
            </option>
            <% }
            } %>
        </select>
        ...
        <select size="1" name="ratingCollapseMinCount">
            ...
            <option value="<%=ratingCollapseCnts[i]%>" selected><%=ratingCollapseCnts[i]%>
            </option>
            <% } else { %>
            <option value="<%=ratingCollapseCnts[i]%>"><%=ratingCollapseCnts[i]%>
            </option>
            <% }
            } %>
        </select>
        ...
        <select size="1" name="ratingCollapseMinMessages">
            ...
            <option value="<%=ratingCollapseMsgs[i]%>" selected><%=ratingCollapseMsgs[i]%>
            </option>
            <% } else { %>
            <option value="<%=ratingCollapseMsgs[i]%>"><%=ratingCollapseMsgs[i]%>
            </option>
            <% }
            } %>
        </select>
