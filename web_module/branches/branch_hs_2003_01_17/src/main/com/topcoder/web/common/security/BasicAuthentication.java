...
public class BasicAuthentication implements WebAuthentication {
     ...
     */
    private void setCookie(long uid) throws Exception {
        String hash = hashForUser(uid);
        Cookie c = new Cookie("user_id", ""+uid+"|"+hash);
        // could set path here, but current layout plus use of the persistor makes it unneccessary
...
log.debug("replacing cookie in old format");
                        Cookie c = new Cookie("user_id", ""+uid+"|"+hash);
...
log.debug("exception parsing cookie", e);
