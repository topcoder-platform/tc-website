...
<div class="pagingBox">
	...
	<logic:notEqual name="<%=Constants.NEXT_IDX_KEY%>" value="-1">
		<a href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=Constants.MODULE%>=<%=Constants.RP_VIEW_STANDINGS%>&<%=Constants.ROUND_ID%>=<%=request.getAttribute(Constants.ROUND_ID)%>&<%=Constants.START_ROW%>=<%=request.getAttribute(Constants.NEXT_IDX_KEY)%>" class="bcLink">next &gt;&gt;</a>
    </logic:notEqual>
</div>
