...
<A HREF="<%=buildHref(request.getParameterMap(), Constants.START_ROW, String.valueOf(startRow+rowCount))%>">next
    page</A>
<%}%>
<%if (startRow > 0) {%>
<A HREF="<%=buildHref(request.getParameterMap(), Constants.START_ROW, String.valueOf(Math.max(0,startRow-rowCount)))%>">previous
    page</A>
<%}%>
<table>
    <tr>
        <td colspan=2></td><td>
        <%if (over && startCol > 0) {%>
        <A HREF="<%=buildHref(request.getParameterMap(), Constants.START_COL, String.valueOf(Math.max(0,startCol-colCount)))%>">
            &lt;&lt;</A>
        <%}%>
    </td>
        ...
        <td align=right>
            ...
            <A HREF="<%=buildHref(request.getParameterMap(), Constants.START_COL, String.valueOf(startCol+colCount))%>">
                &gt;&gt;</A>
            <%}%>
        </td><td></td>
    </tr>
...
%>
        ...
        <%}%>
        <td>
            <A HREF="<%=buildHref(request.getParameterMap(), Constants.SORT_ORDER, sort(sort,"T",primary))%>">Final
                Score</A>
        </td>
        <%
...
%>
        <tr>
            <td>
                <A HREF="longcontest?module=SubmissionHistory&<%=Constants.CODER_ID%>=<%=rec.getCoderID()%>&<%=Constants.ROUND_ID%>=<%=request.getParameter(Constants.ROUND_ID)%>&<%=Constants.ROUND_ID%>=<%=request.getParameter(Constants.ROUND_ID)%>&<%=Constants.COMPONENT_ID%>=<%=request.getParameter(Constants.COMPONENT_ID)%>">+</A>
            </td><td>
