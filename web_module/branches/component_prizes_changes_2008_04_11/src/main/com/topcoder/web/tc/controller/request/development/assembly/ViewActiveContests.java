...
public class ViewActiveContests extends Base {
    ...
    protected void developmentProcessing() throws TCWebException {
        try {
            ...
            r.setContentHandle("active_assembly_contests");
            if (!(sortCol.equals("") || sortDir.equals(""))) {
                r.setProperty(DataAccessConstants.SORT_DIRECTION, sortDir);
                r.setProperty(DataAccessConstants.SORT_COLUMN, sortCol);
            }

            r.setProperty(DataAccessConstants.SORT_QUERY, "active_assembly_contests");
            Map result = getDataAccess().getData(r);
            getRequest().setAttribute("contests", result.get("active_assembly_contests"));
