    ...
    <tc-webtag:iterator id="result" type="com.jivesoftware.forum.QueryResult" iterator='<%=results%>'>
        ...
        <tr>
            <td class="rtThreadCellWrap">
                <a href="?module=Message&messageID=<%=message.getID()%>" class="rtbcLink"><%=ForumsUtil.getMessageSubjectPreview(message, query.getQueryString())%>
                </a>
                <br>
            ...
            <td class="rtThreadCell"><%if (message.getUser() != null) {%>
                ...
                <%}%></td>
            <td class="rtThreadCell" align="right"><%=message.getForumThread().getTreeWalker().getChildCount(message)%>
            </td>
            <td class="rtThreadCell" align="right"><%=ViewCountManager.getInstance().getThreadCount(message.getForumThread())%>
            </td>
            <td class="rtThreadCell"><b>
                <tc-webtag:format object="${result.message.modificationDate}" format="EEE, MMM d yyyy 'at' h:mm a z" timeZone="${sessionInfo.timezone}"/>
            </b></td>
        </tr>
    </tc-webtag:iterator>
</table>
