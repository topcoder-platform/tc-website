<%@  page language="java"  %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
<head>
<title>TopCoder Employment Opportunities</title>

<jsp:include page="../script.jsp" />

</head>

<body>

<jsp:include page="../top.jsp" >
    <jsp:param name="level1" value=""/>
</jsp:include>

<table width="100%" border="0" cellpadding="0" cellspacing="0">
   <tr valign="top">
<!-- Left Column Begins-->
        <td width="180">
            <jsp:include page="../includes/global_left.jsp">
                <jsp:param name="level1" value="my_home"/>
                <jsp:param name="level2" value="employmentOpps"/>
            </jsp:include>
        </td>
<!-- Left Column Ends -->

<!-- Center Column Begins -->
        <td class="cardCell" width="100%" align="center">

        <div class="myTCBody">
        <jsp:include page="../page_title.jsp" >
            <jsp:param name="image" value="employmentOpps"/>
            <jsp:param name="title" value="&#160;"/>
        </jsp:include>

<div align="center">
      <img src="/i/contracting/csfb_logo.gif" border=0/>
</div>

<p>
      <A href="#job1">Quantitative Software Developer</A><br>
      <A href="#job2">Core Infrastructure / Internal Systems Developer</A><br>
      <A href="#job3">Sr. Design Engineer</A>
      <br><br>

<hr noshade="noshade" size="1" />
Global Modelling and Analytics Group, (Architecture and Delivery), Securities Division 
<br><br>
<b>The Firm:</b><br>
Credit Suisse First Boston (CSFB) is a leading global investment bank serving institutional, corporate, government and individual clients.  CSFB's businesses include securities underwriting, sales and trading, investment banking, private equity, financial advisory services and retail online brokerage services.  It operates in 68 locations in 33 countries across 5 continents, and has some 17,000 staff worldwide.  The Firm is a business unit of the Zurich based Credit Suisse Group, a leading global financial services company.  For more information on Credit Suisse First Boston, please visit our corporate website at http://www.csfb.com
<br><br>
Our commitment to providing outstanding service to our clients, our focus on teamwork, diversity and excellence means our recruitment of the best and brightest people is essential to our success.
<br><br>
<b>Division:</b><br>
Credit Suisse First Boston's Securities Division incorporates underwriting, research, sales and trading of a broad range of financial instruments across Fixed Income & Equities.  These include government and corporate bonds, money markets, foreign exchange, U.S. and international equity and equity related securities, precious metals and real estate related assets.  The division also provides a full range of derivative products that address the financing, risk management and investment needs of its customers.  The Securities division services over 5,000 corporate, sovereign and institutional customers worldwide.
<br><br>
<b>Global Modelling and Analytics Group (GMAG):</b><br>
The group develops mathematical models and analytics in the area of derivatives and relative value modelling for all business areas of the Securities Division including Interest Rate Products, Foreign Exchange, Equities, Credit, Local currency, Fund Linked Products and Structured Products
<br><br>
<b>Global Modelling and Analytics Group - Architecture and Delivery (GMAG AD):</b><br>
The goal of this team is to build a partnership with the GMAG Modelers to ensure that GMAG models are delivered to the firm in the most efficient manner. The GMAG AD team focuses on the areas of architecture, common analytics, implementation, and delivery. This includes mapping out the technology strategy for the group, developing advanced tools to automate and bring efficiency to the group's development & testing environment, developing code abstractions for the valuation model framework, building core analytics as well as working with the IT department to integrate GMAG's models into IT applications. Members of this group have significant technology expertise and act as advisors to the GMAG modelers on all technology and coding issues that arise. 
<br><br>
<b>What We Look For:</b><br>
GMAG AD is looking to recruit outstanding individuals for positions in London & NY. All these positions are for candidates with a proven track record in IT. GMAG AD is looking to hire people in 3 distinct areas:
<br><br>

<hr noshade="noshade" size="1" />
<a name="job1"></a>
<span class="bodySubtitle">Quantitative Software Developer</span>
<br><br>
People focusing on the Quantitative projects in GMAG AD typically work on a partnership basis with the modelling teams within GMAG. Their projects range from helping with the integration of GMAG models into risk-management systems (the latter being managed by the IT department), to working with the modellers on technology heavy projects such as designing computationally intensive analytics software, producing & supporting user-friendly pricing sheets or the non-modelling analytics required.  Successful candidates will have strong code design/implementation skills as well as reasonable mathematical skills. 
<br><br>
Key requirements for the role are:
<ul>
<li>An undergraduate degree with honours in a science subject or a mathematical field; including Applied Mathematics, Computer Science, Engineering or Physics.</li>
<li>Strong C++ skills.</li>
<li>Ideal candidate will also have familiarity with COM and the Visual Studio development environment on Windows.</li>
<li>Experience in quantitative development/analytics deployment would be desirable</li>
<li>Good communication skills</li>
<li>Ability to work in a team environment</li>
<li>Financial knowledge and or advanced mathematics are a plus but not a prerequisite.</li>
</ul>
<a href="/tc?module=JobHit&jid=281&jt=4">Register for this position here!</a>
<br/><br/>

<hr noshade="noshade" size="1" />
<a name="job2"></a>
<span class="bodySubtitle">Core Infrastructure / Internal Systems Developer</span>
<br><br>
People focussing on the Core Infrastructure side are responsible for all the key aspects of Infrastructure for GMAG. This includes the development & maintenance of the build/test/release environment, organisation of overnight builds/test monitoring tools, routine code maintenance as well as migration to new compilers. This group also is responsible for making our code base available on non-NT platforms. 
<br><br>
Key requirements for the role are:
<ul>
<li>Strong C++/COM</li>
<li>Good understanding of commercial software development process</li>
<li>Good scripting skills</li>
<li>Knowledge of both windows and Unix environment</li>
<li>Good communication skills</li>
<li>Financial knowledge and or advanced mathematics are a plus but not a prerequisite. </li>
</ul>
<a href="/tc?module=JobHit&jid=282&jt=4">Register for this position here!</a>
<br/><br/>

<hr noshade="noshade" size="1" />
<a name="job3"></a>
<span class="bodySubtitle">Sr. Design Engineer</span>
<br><br>
The design group focuses on building strategic frameworks & efficiency tools for the group. This includes coming up with appropriate abstractions of our models & market data environment to help with the integration of our models into the firms risk-management system.  People in this part of the group need to have a strong understanding of trading/risk systems and also how analytics can be used within them.  
<br><br>
Key requirements for the role are:
<ul>
<li>An undergraduate degree with honours in a science subject or a mathematical field; including Applied Mathematics, Computer Science, Engineering or Physics.</li>
<li>Strong C++/COM</li>
<li>3-5 years or more of experience in financial risk/trading systems design or quantitative code delivery</li>
<li>Knowledge of multithreading issues</li>
<li>Good communication skills</li>
<li>Ability to work in a team environment</li>
<li>Financial knowledge and/or advanced mathematics are a plus but not a prerequisite.</li>
</ul>
<a href="/tc?module=JobHit&jid=283&jt=4">Register for this position here!</a>
<br/><br/>


        </div>
        <p><br/></p>
        </td>
<!-- Center Column Ends -->

<!-- Right Column Begins -->
         <td width="170">
            <jsp:include page="../public_right.jsp">
               <jsp:param name="level1" value="privatelabel"/>
            </jsp:include>
         </td>
<!-- Right Column Ends -->

<!-- Gutter -->
         <td width="10"><img src="/i/clear.gif" width="10" height="1" border="0"></td>
<!-- Gutter Ends -->
    </tr>
</table>

<jsp:include page="../foot.jsp" />

</body>

</html>
