    ...
    <tr>
        ...
        <% if (paginator.getNumPages() > 1) { %>
           ...
           <%  if (paginator.getPreviousPage()) { %>
               <A href="<%=link%>&<%=ForumConstants.CATEGORY_START_IDX%>=<%=paginator.getPreviousPageStart()%>" class="rtbcLink">
                   << PREV</A>&#160;&#160;&#160;
           <%  } %> [
           ...
           %>  <%  if (pages[i] != null) { %>
                   ...
                   <%  } else { %>
                            <A href="<%=link%>&<%=ForumConstants.CATEGORY_START_IDX%>=<%=pages[i].getStart()%>" class="rtbcLink">
                           <%= pages[i].getNumber() %></A>
                   <%  } %>
               <%  } else { %> ... <%  } %>
           <%  } %> ]
           <%  if (paginator.getNextPage()) { %>
               &#160;&#160;&#160;<A href="<%=link%>&<%=ForumConstants.CATEGORY_START_IDX%>=<%=paginator.getNextPageStart()%>" class="rtbcLink">NEXT ></A>
           <%  } %>
        </b></td>
    </tr>
    <%  } %>
