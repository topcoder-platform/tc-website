...
<br><br><br>
The most recent competition in the Intel&#174; Software Network Multi-Threading Competition Series had a rather
different flavor from previous
...
By&#160;
<tc-webtag:handle coderId="15500323"/>
<br>
...
<div align="center"><img src="/i/longcontest/match_editorials/intel_mtcs_3/expi.gif" alt=""/>(0 &lt;= x &lt; letters)
</div>
<br><br>
...
By&#160;
<tc-webtag:handle coderId="7472513"/>
<br>
...
<ul>
    <li>You can have threshold values that decide if a location is almost
    ...
    <li>Given some initial accuracy, you can calculate the property for all
        locations, then check only the <b>n</b> best ones.
    </li>
</ul>
...
<ul>
    <li>Set thresholds to be stricter while risking accuracy. I decided to take
...
(I really admire <b>Iquadrat</b> for <a
        href="http://<%=ApplicationServer.FORUMS_SERVER_NAME%>?module=Message&amp;messageID=536519">finding the simple
    sum</a> of absolute(!) difference for
...
I could code this into three MMX instructions (movq, pcmpeqb, paddb). <a
        href="http://<%=ApplicationServer.FORUMS_SERVER_NAME%>?module=Message&amp;messageID=536416">The trick was</a> to
simply add the results of pcmpeqb's
...
in a single step (virtually SIMDing it, <a
        href="http://<%=ApplicationServer.FORUMS_SERVER_NAME%>?module=Message&amp;messageID=536484">an approach
    <b>roma</b></a> has taken to the extreme!); more
...
submission potentially risked my rank 5 then but I was lucky (<b><a
        href="http://<%=ApplicationServer.FORUMS_SERVER_NAME%>?module=Message&amp;messageID=536528">unlike lquadrat</a></b>)
and could grab the runner-up position.<br><br>

Finally some thoughts about multithreading I had to consider:

<ul>
    <li>8 or 16 threads: 8 threads performed better for me. Most likely the MMX
        and/or the memory load execution unit was almost fully loaded so there was
        no playing field for <a
            href="http://<%=ApplicationServer.FORUMS_SERVER_NAME%>?module=Message&amp;messageID=536525">hyperthreading</a>.
    </li>
