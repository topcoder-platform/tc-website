        ...
        <c:when test="${sessionInfo.anonymous}">
            <div class="topBar">
                <div style="float: right; margin: 5px 0px 0px 0px;">
                    <a class="gMetal" href="http://<%=ApplicationServer.AOLICQ_SERVER_NAME%>/?module=Login">Login</a>
                </div>
            </div>
        </c:when>
        <c:otherwise>
            <div class="topBar">
                <div style="float: right; margin: 5px 0px 0px 0px;">
                    Hello, <tc-webtag:handle coderId="${sessionInfo.userId}" darkBG="true"/>
                        | <a class="gMetal" href="http://<%=ApplicationServer.AOLICQ_SERVER_NAME%>/?<%=Constants.MODULE_KEY%>=Logout">Logout</a>
                </div>
            </div>
        </c:otherwise>
    </c:choose>
