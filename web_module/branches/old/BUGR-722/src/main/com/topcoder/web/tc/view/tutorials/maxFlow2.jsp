...
<A href="/tc?module=Static&d1=tutorials&d2=maxFlow">...read Section 1</A><br />
<br/>

<H3>Section 2</H3>
<span class="bodySubtitle">Max-Flow/Min-Cut Related Problems</span><br>

How to recognize max-flow problems? Often they are hard to detect and usually boil down to maximizing the movement of something from a location to another. We need to look at the constraints when we think we have a working solution based on maximum flow - they should suggest at least an <font face="times new roman, serif"><i>O</i>(<i>N</i>&sup3;)</font> approach. If the number of locations is large, another algorithm (such as dynamic programming or greedy), is more appropriate.
<br/><br/>
...
<br/><br/>
This problem asks us to place a maximum number of rooks on a <font face="times new roman, serif"><i>rows x cols</i></font> chessboard with some squares cut out. The idea behind this might be a little hard to spot, but once this is done, we get into a standard maximum bipartite-matching problem.
<br/><br/>
Notice that at most one rook can be placed on each row or column. In other words, each row corresponds at most to one column where a rook can be placed. This suggests a bipartite matching where set A is composed of elements corresponding to every row of the board, while set B consists of the columns. For each row add edges to every column if the corresponding square is not cut out of the board. Now we can just run maximum bipartite-matching in this network and compute the result. Since there are at most <font face="times new roman, serif"><i>rows &#42; cols</i></font> edges, the time complexity of the algorithm is: <font face="times new roman, serif"><i>O</i>(<i>rows&sup2; &#42; cols</i>)</font>
<br/><br/>
