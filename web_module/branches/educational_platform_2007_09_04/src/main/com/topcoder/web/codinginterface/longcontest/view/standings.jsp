...
<c:if test="${not isIntel}">
        <span class="bodySubtitle">Registrants: <a
                href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?module=ViewRegistrants&amp;<%=Constants.ROUND_ID%>=${infoRow.map['round_id']}"
                class="bcLink">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="statTableHolder">
        <tr>
            <td>
                <table cellpadding="0" cellspacing="0" border="0" width="100%" class="statTable">
                    ...
                    <tr>
                        ...
                        <td class="tableHeader" width="20%" align="center" nowrap="nowrap">
                            <A href="<%=sessionInfo.getServletPath()%>?<tc-webtag:sort column="<%=standings.getColumnIndex("submit_time")%>" includeParams="true" excludeParams="sr;nr"/>">Last
                                Submission Time</A>
                        </td>
                        <td class="tableHeader" width="20%" align="center" nowrap="nowrap">
                        ...
                        <td class="tableHeader" width="20%" align="center" nowrap="nowrap">
                            <A href="<%=sessionInfo.getServletPath()%>?<tc-webtag:sort column="<%=standings.getColumnIndex("test_number")%>" includeParams="true" excludeParams="sr;nr"/>">Example
                                Tests</A>
                        </td>
                        <td class="tableHeader" width="20%" align="center" nowrap="nowrap">
                    ...
                    <tr>
                        ...
                        <td class="<%=even?"statLt":"statDk"%>" align="center">
                            <% if (resultRow.getItem("points").getResultData() != null) { %>
                            <rsc:item name="rank" row="<%=resultRow%>"/>
                            <% } %></td>
                        <td class="<%=even?"statLt":"statDk"%>"
                            align="center"><%if (resultRow.getItem("submit_time").getResultData() != null) { %>
                            <tc-webtag:format object="<%=new Date(resultRow.getLongItem("submit_time"))%>"
                                              format="MM.dd.yyyy HH:mm:ss"/>
                            <% } %></td>
                        <td class="<%=even?"statLt":"statDk"%>" align="center">
                            <tc-webtag:format object="<%=resultRow.getStringItem("language_name")%>"/>
                        </td>
            </td>
            <td class="<%=even?"statLt":"statDk"%>" align="center">
                <% if (resultRow.getIntItem("test_number") > 0) { %>
                <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=Constants.MODULE%>=<%=Constants.RP_EXAMPLE_HISTORY%>&<%=Constants.CODER_ID%>=<rsc:item name="coder_id" row="<%=resultRow%>"/>&<%=Constants.ROUND_ID%>=<%=request.getAttribute(Constants.ROUND_ID)%>&<%=Constants.COMPONENT_ID%>=<rsc:item name="component_id" row="<%=resultRow%>"/>"
                   class="statLink">
            ...
            <td class="<%=even?"statLt":"statDk"%>" align="center">
                ...
                <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=Constants.MODULE%>=<%=Constants.RP_SUBMISSION_HISTORY%>&<%=Constants.CODER_ID%>=<rsc:item name="coder_id" row="<%=resultRow%>"/>&<%=Constants.ROUND_ID%>=<%=request.getAttribute(Constants.ROUND_ID)%>&<%=Constants.COMPONENT_ID%>=<rsc:item name="component_id" row="<%=resultRow%>"/>"
                   class="statLink">
