...
<tc-webtag:rscSelect name="<%=Constants.ROUND_ID%>" list="${rounds}" fieldText="display_name" fieldValue="round_id"
                     onChange="goTo(this)" useTopValue="false"/>
<br/><br/>

<span class="bigHandle">Contest: ${infoRow.map['display_name'] }</span>
<br/>
<span class="bodySubtitle">Problem: <mm:problemLink roundId="${infoRow.map['round_id']}"
                                                    problemId="${infoRow.map['problem_id']}"
                                                    problemName="${infoRow.map['problem_name']}"/> </span><br/>
...
Avg. Submissions: <fmt:formatNumber value="${infoRow.map['avg_submissions']}" minFractionDigits="2"
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="stat">
    <tbody>
        ...
        <tr>
            ...
            <td class="headerC">
                <a href="${sessionInfo.servletPath}?<tc-webtag:sort column="${columnMap['provisional_placed']}" includeParams="true" excludeParams="sr" />">Provisional
                    Rank</a>
            </td>
            <td class="headerR">
                <a href="${sessionInfo.servletPath}?<tc-webtag:sort column="${columnMap['point_total']}" includeParams="true" excludeParams="sr" />">Provisional
                    Score</a>
            </td>
            <td class="headerR">
                <a href="${sessionInfo.servletPath}?<tc-webtag:sort column="${columnMap['system_point_total']}" includeParams="true" excludeParams="sr" />">Final
                    Score</a>
            </td>
            <td class="headerC">
        ...
        <c:forEach items="${competitors}" var="row" varStatus="status">
            <tr class='${status.index % 2 == 1? "dark" : "light" }'>
                ...
                <td class="value">
                    <tc-webtag:handle context='marathon_match' coderId="${row.map['coder_id']}"/>
                </td>
                <td class="valueC">${row.map['provisional_placed']}</td>
                <td class="valueR">
                    <fmt:formatNumber value="${row.map['point_total']}" minFractionDigits="2" maxFractionDigits="2"/>
                </td>
                <td class="valueR">
                    <fmt:formatNumber value="${row.map['system_point_total']}" minFractionDigits="2"
                                      maxFractionDigits="2"/>
                </td>
                <td class="valueC">${row.map['language_name']}</td>
                <td class="valueC" nowrap="nowrap"><a
                        href="/longcontest/stats/?<%=Constants.MODULE%>=ViewSystemTestResults&amp;<%=Constants.ROUND_ID%>=${roundId}&amp;<%=Constants.PROBLEM_ID%>=${row.map['problem_id']}&amp;<%=Constants.CODER_ID%>=${row.map['coder_id']}">
                    results</a></td>
                <td class="valueC" nowrap="nowrap"><a
                        href="/longcontest/?<%=Constants.MODULE%>=ViewSubmissionHistory&amp;<%=Constants.ROUND_ID%>=${roundId}&amp;<%=Constants.PROBLEM_ID%>=${row.map['problem_id']}&amp;<%=Constants.CODER_ID%>=${row.map['coder_id']}">
                    submission history</a></td>
                <td class="valueC" nowrap="nowrap"><a
                        href="/longcontest/?<%=Constants.MODULE%>=ViewExampleHistory&amp;<%=Constants.ROUND_ID%>=${roundId}&amp;<%=Constants.PROBLEM_ID%>=${row.map['problem_id']}&amp;<%=Constants.CODER_ID%>=${row.map['coder_id']}">
