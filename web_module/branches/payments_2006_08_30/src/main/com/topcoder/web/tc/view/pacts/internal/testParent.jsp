...
<br> 
<%
	BasePayment drp=new DigitalRunPrizePayment(20009005, 780, 4);
	pcs.addPayment(drp);
%>

Base Digital Run Prize id:  <%= drp.getId() %>
...
<br> 
<%
	BasePayment p=new CoderReferralPayment(drp, 7545675, 78.0);
	pcs.addPayment(p);
%>
getId: <%= p.getId() %> <br>
...
<br> 
<%
	p=new ReliabilityBonusPayment(drp, 39.0);
	pcs.addPayment(p);
%>
getId: <%= p.getId() %> <br>
getCoderId: <%= p.getCoderId() %> <br>
