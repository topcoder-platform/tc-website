...
<rss version="2.0"
    ...
    <channel>
        ...
        <link>
            <jsp:getProperty name="sessionInfo" property="absoluteServletPath"/>
        </link>
        <description>Most recent forum messages</description>
        <language><%= JiveGlobals.getLocale().getLanguage() %>
        </language>
        ...
        <tc-webtag:iterator id="message" type="com.jivesoftware.forum.ForumMessage" iterator='<%=(Iterator)request.getAttribute("messages")%>'>
            ...
            <item>
                <title><%= StringUtils.escapeForXML(message.getSubject()) %>
                </title>
                <link>
                    <jsp:getProperty name="sessionInfo" property="absoluteServletPath"/>
                    ?module=Message&amp;<%=ForumConstants.MESSAGE_ID%>=
                    <jsp:getProperty name="message" property="ID"/>
                </link>
                <description><![CDATA[<%= message.getBody() %>]]></description>
                <dc:creator><%= message.getUser().getUsername() %>
                </dc:creator>
                <category><![CDATA[<%= message.getForum().getName() %>]]></category>
                <pubDate><%= formatter.format(message.getCreationDate()) %>
                </pubDate>
                <jf:creationDate><%= formatter.format(message.getCreationDate()) %>
                </jf:creationDate>
                <jf:modificationDate><%= formatter.format(message.getModificationDate()) %>
                </jf:modificationDate>
                <jf:author><%= message.getUser().getUsername() %>
                </jf:author>
                <jf:replyCount><%= (message.getForumThread().getTreeWalker().getChildCount(message)) %>
                </jf:replyCount>
            </item>
            <% } %>
        </tc-webtag:iterator>
