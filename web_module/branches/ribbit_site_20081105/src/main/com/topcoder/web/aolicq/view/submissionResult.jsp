...
<jsp:include page="aolicqTop.jsp"/>
        ...
        <div id="container">
            ...
            <div id="content">
                ...
                <div class="linkBox"><aolicq:forumLink forumID="${submission.contest.forumId}"/></div>
            ...
            <c:when test="${submission.contest.viewableSubmissions.value}">
                <c:choose>
                    <c:when test="${submission.mimeType.fileType.imageFile}">
                        <aolicq_tags:submissionDisplay submissionId="${submission.id}" width="${submission.width}" height="${submission.height}"/>
                    </c:when>
                    <c:otherwise>
                        ...
                        <a href="${sessionInfo.servletPath}?<%=Constants.MODULE_KEY%>=DownloadSubmission&amp;<%=Constants.SUBMISSION_ID%>=${submission.id}">
                            <img src="/i/events/aolicq/interface/magnify.png" alt="" onmouseover="popUp(this,'popIt')" onmouseout="popHide()"/>
                        </a>
                    </c:otherwise>
                </c:choose>
            </c:when>
            <c:otherwise>
                <img src="/i/events/aolicq/interface/magnifyFade.png" alt="" />
            </c:otherwise>
        </c:choose>
