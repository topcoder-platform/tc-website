...
public class ContestPropertyDAOHibernateTestCase extends TCHibernateTestCase {
    ...
    public void testFind_Integer() {
        ...
        for (int i = 0; i < contestPropertiesCnt; i++) {
            ContestProperty property = this.testedInstance.find(this.ids[i]);
            Assert.assertNotNull("The existing contest property is not found", property);
            Assert.assertEquals("Wrong contest property ID retrieved", this.ids[i], property.getId());
            Assert.assertEquals("Wrong contest property description retrieved",
                                this.descriptions[i], property.getDescription());
        }
    }
    ...
    public void testGetContestTypes() {
        ...
        for (int i = 0; i < contestPropertiesCnt; i++) {
            ...
            for (ContestProperty returnedProperty : (List<ContestProperty>) properties) {
                if (returnedProperty.getId().equals(this.ids[i])) {
                    found = true;
                    Assert.assertEquals("Wrong contest property description retrieved",
                                        this.descriptions[i], returnedProperty.getDescription());
                }
            }
            Assert.assertTrue("The contest property " + this.ids[i] + " is not retrieved", found);
        }
    }
}
