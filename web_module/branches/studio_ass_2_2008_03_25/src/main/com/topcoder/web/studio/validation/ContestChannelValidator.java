...
public class ContestChannelValidator implements Validator {
    ...
    public ValidationResult validate(ValidationInput input) {
        ValidationResult ret = new NonEmptyValidator("Please select a contest channel.").validate(input);
        if (ret.isValid()) {
            try {
                ...
                if (channel == null) {
                    return new BasicResult(false, "Please select valid contest channel.");
                } else {
