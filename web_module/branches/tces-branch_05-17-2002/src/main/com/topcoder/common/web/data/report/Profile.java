...
public class Profile implements Serializable {
  ...
  public Profile(String handle, String firstName, String lastName) {
    if (Conversion.checkNull(handle).equals("")) this.handle = "%";
    else this.handle = handle.trim();
    if (Conversion.checkNull(firstName).equals("")) this.firstName = "%";
    else this.firstName = firstName.trim();
    if (Conversion.checkNull(lastName).equals("")) this.lastName = "%";
  ...
  public void execute() throws Exception {
    ...
    if (a.size() == 0) throw new Exception("Could not find user with handle: " + handle + " first name: " + firstName + " last name: " + lastName);
    else generalInfo = (ResultItem[])a.get(0);
