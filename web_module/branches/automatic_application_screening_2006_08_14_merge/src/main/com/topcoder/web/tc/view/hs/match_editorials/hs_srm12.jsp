...
Let's start at year one and iterate years until we reach one with given indict, circle to the Sun and circle to the Moon.</p>
But if you want to win the Shortest-250-Competition, so popular at the TopCoder forums, you should use another approach, based on the
<a href = "http://en.wikipedia.org/wiki/Chinese_theorem" target=blank>Chinese remainder theorem</a>. Note, that since 15, 19 and 28 are coprime numbers, the Indict system establishes the one-to-one correspondence between years in range from 1 to 15*19*28 = 7980 and its indict notation.
<br><br>
Let <i>inv(a, b)</i> be such an integer c, that <i>ac mod b = 1</i>.
Consider the expression: <br><br>
<i>Y = inv(28*19, 15)*28*19*(indict-1) + inv(15*19, 28)*15*19*(circleSun-1) + inv(28*15, 19)*28*15*(circleMoon-1)</i><br>
...
&nbsp;&nbsp;&nbsp;&nbsp;<font color=blue>int</font> getYear(<font color=blue>int</font> indict,<font color=blue>int</font> circleSun,<font color=blue>int</font> circleMoon){<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=blue>return </font> (6916*indict+4845*circleSun+4200*circleMoon-1)%7980+1;<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
};
</pre>
...
<pre><font color=blue>string</font> ApplyRotation(<font color=blue>string</font> die, <font color=blue>int</font>[] permutation){<br>
...
one with the lexicographically first notation. Let's call such a notation a <i>normal form</i> of the die. In this case, we can compare
...
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;<i><font color = green>//now all the rotations with side 1, 5, 0, and 4 at front are handled</font></i><br><br>
&nbsp;&nbsp;&nbsp;&nbsp;die = ApplyRotation(die, rotateBack);<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;die = ApplyRotation(die, rotateRight);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=blue>if</font>(die &lt; result)result = die;<br>
&nbsp;&nbsp;&nbsp;&nbsp;}<br>
&nbsp;&nbsp;&nbsp;&nbsp;<font color=blue>return</font> result;<br>
}
</pre>

Another approach of recursive generation of the rotations was used by <tc-webtag:handle coderId="22630491" context="algorithm"/>. See his <a href = "/tc?module=HSProblemSolution&cr=22630491&rd=10064&rm=249463&pm=6512">solution</a> for the details.<br>
<br>
<br>
...
<p>
In this problem we need to use dynamic programming twice (read <a href="/tc?module=Static&d1=tutorials&d2=dynProg">this</a> tutorial if you are unfamiliar with it).
To compute the probability of the knights' win, we need to predict the dragon's behavior.
To do this, we need to know when it is worthwhile for the dragon to attack and when it is better to skip and hide in the cave.
At first, for all possible amounts of knights and dragon's heads 
that may appear after dragon's turn, let's determine the probability of knight's victory. Let K[i][j] be the probability of victory of <i>i</i> knights over <i>j</i>-headed dragon. This is quite simple - see the following pseudocode:</p>
...
In the first case knights will win with probability of D[a][b - 1], and in the second their win will occur with probability of D[a - 1][b].
This means, the cumulative probability of their win is Pab = (<b>probDragon</b>* D[a][b - 1] + (1 - <b>probDragon</b>) * D[a - 1][b]).
Therefore, the dragon will attack only if Pab will be smaller than K[a][b], since he wants to minimize the knights' chances to win.
...
&nbsp;&nbsp;&nbsp;<font color=blue>for</font> j = 1 <font color=blue>to</font> h <font color=blue>do</font><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;D[i][j] = <font color=blue>min</font>(K[i][j], probDragon * D[i-1][j] + (1 - probDragon) * D[i][j-1]);<br>
</pre>

You can also see <tc-webtag:handle coderId="21111148" context="algorithm"/>'s <a href = "/tc?module=HSProblemSolution&cr=21111148&rd=10064&rm=249462&pm=6694">solution</a> for 
a clear implementation of this approach. As an implementation detail, you can solve the problem with only one two-dimensional array. 
...
<pre><font color=blue>for</font> i = 1 <font color=blue>to</font> k <font color=blue>do</font><br>
&nbsp;&nbsp;&nbsp;<font color=blue>for</font> j = 1 <font color=blue>to</font> h <font color=blue>do</font><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;K[i][j] = <font color=blue>min</font>(K[i][j], probDragon * K[i-1][j] + (1 - probDragon) * K[i][j-1]);<br>
</pre>


  <img src="/i/m/Pawa_big.jpg" alt="" width="55" height="61" border="0" hspace="6" vspace="1" align="left" class="myStatsPhoto"/><br />
