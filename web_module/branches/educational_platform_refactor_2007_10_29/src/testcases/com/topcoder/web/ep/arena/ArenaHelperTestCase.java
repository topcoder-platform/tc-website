    ...
    public final void testAddNewAssignment() {
        ...
        for (RoundSegment rs : r.getRoundSegments()) {
            if (rs.getId().equals(1)) {
                ...
                if (rs.getId().equals(2)) {
                    assertTrue("(2) Wrong segment start time ", rs.getStartTime().equals(basicADTO.getStartDate()));
                } else {
                    assertTrue("(3) Wrong segment start time ", rs.getStartTime().equals(basicADTO.getEndDate()));
                }
...
//        ah.updateRoundRegistration(roundId, coderIds);
    }
