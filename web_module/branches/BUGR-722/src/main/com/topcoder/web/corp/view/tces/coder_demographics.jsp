        ...
        <td class=recruitingBody>
            ...
            <table cellspacing="0" cellpadding="0" width="100%" class="screeningFrameNB">
                <tr valign="top">
                    <td class=bodyText>
                        <div style="float:right;">
                            <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=TCESConstants.TASK_PARAM%>=ViewSearchTask&<%=TCESConstants.CAMPAIGN_ID_PARAM%>=<jsp:getProperty name="CoderDemographicsTask" property="campaignID" />">Search
                                candidates</A>
                        </div>
                        ...
                        <strong>Name:</strong> <%= CoderDemographicsTask.getMemberInfo().getItem(0, "first_name").toString()%> <%= CoderDemographicsTask.getMemberInfo().getItem(0, "middle_name").toString()%> <%= CoderDemographicsTask.getMemberInfo().getItem(0, "last_name").toString()%>
                        <br>
                        ...
                        <strong>Email:</strong>
                        <a href="mailto:<%=CoderDemographicsTask.getMemberInfo().getItem(0, "email").toString() %>" class="bodyText"><%= CoderDemographicsTask.getMemberInfo().getItem(0, "email").toString() %>
                        </A><br>
                        <strong>Phone:</strong> <%= CoderDemographicsTask.getMemberInfo().getItem(0, "home_phone").toString()%>
                        <br>
                        ...
                        <jsp:getProperty name="CoderDemographicsTask" property="jobName"/>
                        <br><br>
                        <% if (CoderDemographicsTask.getIsRanked()) { %>
                        Coder Demographic Info
                        <br>
                        <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=TCESConstants.TASK_PARAM%>=<%=TCESConstants.COMPETITION_HISTORY_TASK%>&<%=TCESConstants.CAMPAIGN_ID_PARAM%>=<%=CoderDemographicsTask.getCampaignID()%>&<%=TCESConstants.JOB_ID_PARAM%>=<%=CoderDemographicsTask.getJobID()%>&<%=TCESConstants.MEMBER_ID_PARAM%>=<%=CoderDemographicsTask.getMemberID()%>" class="bodyText">Coder
                            Competition History</A>
                        <br>
                        <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=TCESConstants.TASK_PARAM%>=<%=TCESConstants.PROBLEM_SUBMISSIONS_TASK%>&<%=TCESConstants.CAMPAIGN_ID_PARAM%>=<%=CoderDemographicsTask.getCampaignID()%>&<%=TCESConstants.JOB_ID_PARAM%>=<%=CoderDemographicsTask.getJobID()%>&<%=TCESConstants.MEMBER_ID_PARAM%>=<%=CoderDemographicsTask.getMemberID()%>" class="bodyText">Coder
                            Problem Submissions</A>
                        <br>
                        <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=TCESConstants.TASK_PARAM%>=PlacementInfoTask&<%=TCESConstants.CAMPAIGN_ID_PARAM%>=<%=CoderDemographicsTask.getCampaignID()%>&<%=TCESConstants.JOB_ID_PARAM%>=<%=CoderDemographicsTask.getJobID()%>&<%=TCESConstants.MEMBER_ID_PARAM%>=<%=CoderDemographicsTask.getMemberID()%>" class="bodyText">Coder
                            Skill Set</A>
                        <br><br>
                        <% } %>
                    </td>
                ...
                <tr>
                    ...
                    <td class="bodyText" align="right">
                        <% if (!CoderDemographicsTask.isRestrictedCampaign()) {%>
                        <A HREF="JavaScript:getGraph('/corp/graph?c=rating_history_graph&cr=<jsp:getProperty name="CoderDemographicsTask" property="memberID"/>&cm=<jsp:getProperty name="CoderDemographicsTask" property="companyId"/>','800','600','history')" class="bodyText">Rating
                            History Graph</a> &nbsp;&nbsp;|&nbsp;&nbsp;
                        <% } %>
                        <A HREF="JavaScript:getGraph('/corp/graph?c=rating_distribution_graph&cm=<jsp:getProperty name="CoderDemographicsTask" property="companyId"/>','600','400','distribution')" class="bodyText">Overall
                            Rating Distribution Graph</a>
                    </td>
                </tr>
            </table>
            ...
            <table cellspacing="0" cellpadding="0" width="100%" class="screeningFrame">
                ...
                <tces:mapIterator id="question" mapList="<%=(List)CoderDemographicsTask.getQuestionList()%>">
                    ...
                    <tr>
                        <td class="<%=i%2==1?"screeningCellOdd":"screeningCellEven"%>"><%= (String) question.get("question") %>
                        </td>
                        <td class="<%=i%2==1?"screeningCellOdd":"screeningCellEven"%>" align=right nowrap=nowrap><%= (String) question.get("response") %>
                        </td>
                    </tr>
                </tces:mapIterator>
