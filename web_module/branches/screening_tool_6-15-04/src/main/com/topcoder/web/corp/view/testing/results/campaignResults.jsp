        ...
        <td width="100%" align="center"><img src="/i/corp/clear.gif" width="200" height="11" alt="" border="0"><br>
            ...
            <table cellspacing="0" cellpadding="0" width="600" class="screeningFrame">
                ...
                <tr>
                    ...
                    <td class='<%=cssClasses[counter % 2]%>' align=center>
                        <A href='?<%=Constants.MODULE_KEY%>=ProblemResult&<%=Constants.PROBLEM_ID%>=<%=row.getStringItem("problem_id")%>&<%=Constants.SESSION_ID%>=<%=row.getStringItem("session_round_id")%>&<%=Constants.PROBLEM_TYPE_ID%>=<%=row.getStringItem("problem_type_id")%><%--&<%=Constants.ROUND_ID%>=<%=row.getStringItem("session_round_id")%>--%>'>
                            view
                        </A>
                    </td>
