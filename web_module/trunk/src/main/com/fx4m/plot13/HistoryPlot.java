...
/*****************************************************************************/
{
 ...
 static final Color redBorderColor   = new Color(255,255,255);
 static final Color whiteBorderColor = new Color(255,255,255);
...
for(int i=0;i<d.length;i++)
   ...
   if (y>1900)  db[i]=y+(Integer.parseInt(t.nextToken())-1)/12.0+
                        (Integer.parseInt(t.nextToken())-1)/12.0/31 ;
   else         db[i]=(y-1)/12.0+(Integer.parseInt(t.nextToken())-1)/12.0/31+
...
for(int i=0;i<c.length;i++)
   ...
   if ((i==0||i==c.length) &&
       ...
       c[i].get(c[i].HOUR_OF_DAY)==0) db[i]+=0.5/12.0/31.0;
   }
return plot(r,db,n);
}
/*****************************************************************************/
...
for(int i=0;i<d.length;i++)
   ...
   if ((i==0||i==d.length) &&
       ...
       c.get(c.HOUR_OF_DAY)==0) db[i]+=0.5/12.0/31.0;
   }
return plot(r,db,n);
}
/*****************************************************************************/
...
/*****************************************************************************/
{
 ...
 if ( data.length == 0 ) 
    { // should figure out current year here, oh well
    data=new int [2] ;time=new double[2]; 
 ...
 if ( data.length == 1 ) 
    {
    int []ndata=new int [3] ;double []ntime=new double[3]; 
    ntime[0]=(int)(time[0]);   ndata[0]=0;
    ntime[1]=time[0];          ndata[1]=data[0];
    ntime[2]=(int)(time[0]+1); ndata[2]=0;
...
{ // set pixel in databuffer to specified color
...
/*****************************************************************************/
 ...
 if ( doborder )
    ...
    for ( x = 0 ; x < w ; x ++ ) 
       { // turn bottom border upside down
       y=h-1; setpx(x,y,128,0,0);
       if(x>0&&x<w-1){y=h-2; setpx(x,y,200,0,0);}
       if(x>bw-2&&x<w-bw+1){y=h-bw+1; setpx(x,y,255,128,128);}
       if(x>bw-3&&x<w-bw+2){y=h-bw+2; setpx(x,y,255,64,64);}
       }
    }
 g.setTransform(save);
 g.scale(0.5,0.5) ;
 ...
 g.drawString("Rating History",w/5-25,20);
 g.setTransform(save);
...
/*****************************************************************************/
 ...
 mind -= 100 ;
 if ( datapoints < 2 ) { maxd=3500;mind=-100; }
...
/*****************************************************************************/
 ...
 for(x=(int)time[0];x<=(int)(time[time.length-1]);x++ )
    ...
    double xy ; xy = 0 ;
    if ( x < (int)lt && x > (int)ft ) xy = x+0.5 ;
    else if ( x == (int) ft && (ft-(int)ft)*xscale>26 ) xy = (ft+(int)ft+1)/2 ;
    else if ( x == (int) ft && (ft-(int)ft)*xscale>13 ) xy = ft ;
    else if ( x == (int) lt && (lt-(int)lt)*xscale>26 ) xy = (lt+(int)lt)/2 ;
    else if ( x == (int) lt && (lt-(int)lt)*xscale>13 ) xy = lt ;
    else xy = 0 ;
...
// every 100 pts a pixel in lut is twice as bright, makes horz lines
 ...
 int ddy ; ddy=(maxd-mind)>2000? 200 : 100 ;
 ...
 else for(i=0;i<data.length-1;i++)
    {
 ...
 for ( x = pb+1 ; x < w-pb-1 ; x ++ )
    ...
    for ( y = h-pb-1 ; y>pb ; y -- )
       {  // curve pixels are bright white
       int r,g;
       if ( (b=db.getElem(3*(y*w+x)+0))+
            (g=db.getElem(3*(y*w+x)+1))+
            (r=db.getElem(3*(y*w+x)+2)) > 700 ) in = false ;
       else if ( in ) 
          {
 ...
 for(i=0;i<data.length;i++) // put roundish markers for each match
    {
    x=(int)(time[i]*xscale+xoff+0.5); y=(int)(data[i]*yscale+yoff-0.5);
    for(int z=x-2;z<x+3;z++)for(int u=y-2;u<y+3;u++)
       if (datapoints > 0 ) 
