...
<jsp:useBean id="QueryListTask" scope="request" class="com.topcoder.web.query.bean.task.QueryListTask" />
    ...
    <TR>
      <TD WIDTH="170" BGCOLOR="#000000" VALIGN="top">
        <jsp:include page="left.jsp" flush="true">
          <jsp:param name="<%=Constants.DB_PARAM%>" value="<%=QueryListTask.getDb()%>"/>
        ...
        <TABLE WIDTH="60%" BORDER="0" CELLPADDING="0" CELLSPACING="0">
          ...
          <TR>
            ...
            <TD CLASS="statText" ALIGN="left" COLSPAN="2">
              <jsp:getProperty name="QueryListTask" property="Db"/>
            </TD>
          </TR>
          ...
          <query:resultSetRowIterator id="query" list="<%=QueryListTask.getQueryList()%>">
            <TR>
              <TD CLASS="statText" ALIGN="right">
                <A HREF="<jsp:getProperty name="SessionInfo" property="ServletPath"/>?<%=Constants.TASK_PARAM%>=<%=Constants.MODIFY_QUERY_TASK%>&<%=Constants.DB_PARAM%>=<jsp:getProperty name="QueryListTask" property="Db"/>&<%=Constants.QUERY_ID_PARAM%>=<query:resultSetItem row="<%=query%>" name="query_id"/>" CLASS="statText">
                  [edit]
                </A>
                &#160;&#160;
              </TD>
              <TD CLASS="statText">
                <A HREF="<jsp:getProperty name="SessionInfo" property="ServletPath"/>?<%=Constants.TASK_PARAM%>=<%=Constants.QUERY_DETAIL_TASK%>&<%=Constants.DB_PARAM%>=<jsp:getProperty name="QueryListTask" property="Db"/>&<%=Constants.QUERY_ID_PARAM%>=<query:resultSetItem row="<%=query%>" name="query_id"/>" CLASS="statText">
