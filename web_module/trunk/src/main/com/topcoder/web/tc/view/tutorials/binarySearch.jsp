...
<br><br>
The second part is proving that binary search can be applied to the predicate. This is where we use the main theorem, verifying that the conditions laid out in the theorem are satisfied. The proof doesn't need to be overly mathematical, you just need to convince yourself that <i>p</i>(x) implies <i>p</i>(y) for all y > x or that Â<i>p</i>(x) implies Â<i>p</i>(y) for all y < x. This can often be done by applying common sense in a sentence or two.
<br><br>
When the domain of the predicate are the integers, it suffices to prove that <i>p</i>(x) implies <i>p</i>(x+1) or that Â<i>p</i>(x) implies Â<i>p</i>(x-1), the rest then follows by induction.
<br><br>
