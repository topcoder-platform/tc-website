...
<br>
The begining of the match belonged to <tc-webtag:handle coderId="11972352" context="hs_algorithm"/>, who got the fastest solution on the easy problem and the second fastest on the medium, giving him an impressive 744.01 out of 750 points. Many other coders were not far behind, however, and the final result came down to their performance on the tricky 1,000 point problem.
After the end of the coding phase <tc-webtag:handle coderId="22375930" context="hs_algorithm"/> held first place with a great score of 1563.93, <tc-webtag:handle coderId="22268848" context="hs_algorithm"/>
was second with 1491.21, followed by
<tc-webtag:handle coderId="20812309" context="hs_algorithm"/>, with <tc-webtag:handle coderId="21111148" context="hs_algorithm"/> right after him, both with over 1480 points.
After the challenge phase  <tc-webtag:handle coderId="22375930" context="hs_algorithm"/> was still in first, with an extra 25 points. <tc-webtag:handle coderId="20812309" context="hs_algorithm"/>
moved to second, thanks to 75 points from challenges, and <tc-webtag:handle coderId="20331303" context="hs_algorithm"/> advanced to third after grabbing 225 challange points.  It's worth noting that both  <tc-webtag:handle coderId="20812309" context="hs_algorithm"/> and 
...
It doesn't seem difficult to code, and yet many, even high rated coders, got it wrong. Most of the failed solutions weren't wrong answers, but exceptions when one reaches out of the array or string bounds, for testcases like "h". You can also look at the nice <tc-webtag:handle coderId="12005484" context="hs_algorithm"/> <a href = "/tc?module=HSProblemSolution&cr=12005484&rd=10066&rm=249645&pm=6720">solution</a> that uses set, in place of array, for marking letters.
<br><br>
...
<br><br><i>Given an undirected graph, find the number of <a href ="http://en.wikipedia.org/wiki/Connected_component_(graph_theory)">connected components</a>.</i>
<br><br>
Most of the coders used dfs to mark all the vertices of the connected component. The answer in that case is the number of the times you need to run dfs to cover the whole graph. The fastest <a href="/tc?module=HSProblemSolution&cr=8471704&rd=10066&rm=249649&pm=6721">submission</a> by <tc-webtag:handle coderId="8471704" context="hs_algorithm"/> uses exaclty this idea.
<br><br>
...
This approach can be seen in<tc-webtag:handle coderId="22629916" context="hs_algorithm"/>'s  <a href="/tc?module=HSProblemSolution&cr=22629916&rd=10066&rm=249650&pm=6721">code</a>.
<br><br>
...
<p>
After taking a glance at the constraints one should realize that, with a maximum of six teams, a bruteforce solution won't have a problem running in time.
...
<p>
To address the first point you can assign the unique representation to every round. One way to do that is representing it as a list of <b>n/2</b> pairs,
where each pair has the lower index on the first position and additionally pairs are sorted by the first element.
In example:
<pre>
 A     B     C
3-5   1-2   0-4
...
All A,B,C represent the same round, but only C is corresponding to the above constraints. Please note that every round setting can be uniquely represented 
in this fashion. So generating only rounds in this format will ensure that you will count each of the schedules exactly once. Now generating all the schedules can be done
...
You can take a look on the <a href="/tc?module=HSProblemSolution&cr=22375930&rd=10066&rm=249648&pm=6722">
fastest solution</a> by <tc-webtag:handle coderId="22375930" context="hs_algorithm"/>, who used this approach.
</p>

  <img src="/i/m/slex_big.jpg" alt="" width="55" height="61" border="0" hspace="6" vspace="1" align="left" class="myStatsPhoto"/><br />
