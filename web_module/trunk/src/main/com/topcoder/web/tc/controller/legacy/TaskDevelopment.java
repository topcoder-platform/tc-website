...
*/
            ...
            if (command.equals("tcs_inquire") || command.equals("tcs_app_inquire")) {
                if (nav.isLoggedIn()) {
                    ...
                    devTag.addTag(new ValueTag("projectId", request.getParameter("projectId")));
                    if(!tcoTermsCheck(nav.getSessionInfo().getUserId()) && (request.getParameter("continue") == null || !request.getParameter("continue").equals("true")))
                    {
...
//                devTag.addTag(Base.getProjectTotals().getTag("Project", "Total"));
    ...
    public static boolean tcoTermsCheck(long userId) throws Exception {
        ...
        log.info(new GregorianCalendar(104, 7, 1).toString());
        if(c.before(new GregorianCalendar(104, 7, 1)) || c.after(new GregorianCalendar(104, 9, 23)))
        {
            return true;
        } 
