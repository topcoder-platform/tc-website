...
<c:set var="url"
       value="/tc?${requestScope.MODULE_KEY}=Static&amp;d1=tournaments&amp;d2=tco11&amp;d3=overview&amp;d4=${page}"/>

<c:choose>
    <c:when test="${page eq 'registrants'}">
        <c:set var="url" value="/tco11?${requestScope.MODULE_KEY}=Registrants"/>
    </c:when>
    <c:when test="${page eq 'registration'}">
        <c:set var="url" value="/tco11?${requestScope.MODULE_KEY}=ViewUnifiedRegistration"/>
    </c:when>
</c:choose>
