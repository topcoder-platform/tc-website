...
TopCoder uses its own UML tool to capture diagrams during the design process.  You can <a href="/tc?module=Static&amp;d1=dev&amp;d2=umltool&amp;d3=description&amp;node=dev_uml_tool">download</a> the TopCoder UML Tool for free from the TopCoder website.
<br /><br />
...
<ul>
<li><strong>Use Case</strong> - You will be assigned a set of use cases to implement.  The use case diagram lays out all of the use cases for the application.</li>
<li><strong>Activity Diagrams</strong> - The activity diagrams correspond directly to the use cases.  Each Activity Diagram is named after the Use Case in the following format: <em>Use Case Name Activity Diagram Name</em>.  The purpose of this document is to record the logical use case flow.</li>
<li><strong>Requirement Specification</strong> - The requirement specification is formatted based on Use Cases.  Each use case is defined under the logical requirements section and is numbered as 2.X for each use case.  2.X.Y corresponds to each activity diagram.</li>
<li><strong>Sequence Diagram</strong> - Each sequence diagram is named after the activity diagram it corresponds to.  These sequence diagrams will be the map to your assembly work.  Each sequence diagram will explain to the method level how to assemble the components to meet the functionality required for the use case.  If the sequence diagram cannot be followed exactly, participants are required to raise the issue to TopCoder via the contest forum.  You will be required to implement all <span style="color:#FF9900;">orange</span> colored objects.</li>
<li><strong>Component Interface Diagram</strong> - This diagram defines the required interfaces for all components of the application.  As an assembly participant, you will implement all of the required interfaces exactly as specified.  Test cases will be developed based on these specific interfaces.</li>
<li><strong>Design Specification</strong> - This document will detail and define all auditing, logging, and threading information for the system.  Additionally, you will find supplementary information to implement your prototype conversion.</li>
<li><strong>Component Distributions</strong> - Download the complete implementation for all components to be used in the competition.</li>
</ul>
...
<ul>
<li><a href="/tc?module=Static&amp;d1=dev&amp;d2=support&amp;d3=assemblyUnitTestsCSharp">Writing C# Unit Tests</a></li>
<li><a href="/tc?module=Static&amp;d1=dev&amp;d2=support&amp;d3=assemblyUnitTestsJava">Writing Java Unit Tests</a></li>
</ul>
...
<li>ALWAYS document your test code as thoroughly as you document your component code.</li>
...
<li>Break up your tests within those classes into the smallest functions possible; that way, it is clear which areas of the component are failing. You will then be able to use the number of tests passing and failing as a completion metric.</li>
...
<li>Test expected component processes: for example, loading, processing data, and saving data.</li>
<li>Don't forget to clean up your environment! Unit tests should leave the system in the same state they found it in; there should be no persistent changes. This is checked during review.</li>
