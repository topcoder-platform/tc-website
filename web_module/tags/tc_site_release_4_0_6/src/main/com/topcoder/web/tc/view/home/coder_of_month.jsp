<%@ taglib uri="tc.tld" prefix="tc" %>
<%@ taglib uri="tc-webtags.tld" prefix="tc-webtag" %>
<table width="180" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td><img src="/i/coder_month_top.gif" alt="" width="180" height="11" border="0"></td>
    </tr>

    <tr>
        <td><img src="/i/header_coder_month.gif" alt="Coder of the Month" width="180" height="20" border="0"></td>
    </tr>
</table>

<table width="180" border="0" cellspacing="0" cellpadding="5" bgcolor="#555555">
    <tr>
        <td class="statTextBig" bgcolor="#333333">November:&#160;<br><tc-webtag:handle coderId="293874" /></td>
        <td class="statTextBig" bgcolor="#333333" align="right">&#160;<br><A href="/tc?module=SimpleStats&trans=true&c=com_history&d1=statistics&d2=coderOfMonth" class="statTextBig">Archive</A></td>
    </tr>
</table>

<table width="180" border="0" cellspacing="0" cellpadding="3" bgcolor="#222222">
<%--
--%>
    <tr>
        <td class="formHandleEven">2005 TopCoder Open<br><br>Design Champion<br>&<br>Algorithm Finalist</td>
        <td align="center" valign="middle"><img src="/i/home/nicka81_com.jpg" alt="" border="0" class="myStatsPhoto"/></td>
    </tr>
<%--
    <tr>
        <td align="left" background="/i/graybv_lite_bg.gif" class="statText">Contest</td>
        <td align="right" background="/i/graybv_lite_bg.gif" class="statText">Rating</td>
        <td align="right" background="/i/graybv_lite_bg.gif" class="statText"></td>
    </tr>
    <tr>
        <td class="formHandleEven">TCO05 RD 3</td>
        <td class="formHandleEven" align="right">3004</td>
        <td rowspan="9" align="center" valign="middle"><img src="/i/m/ivan_metelsky_mug.gif" alt="" border="0" class="myStatsPhoto"/></td>
    </tr>
    <tr>
        <td class="formHandleEven">SRM 265</td>
        <td class="formHandleEven" align="right">1911</td>
    </tr>
    <tr>
        <td class="formHandleEven">SRM 264</td>
        <td class="formHandleEven" align="right">1805</td>
    </tr>
    <tr>
        <td class="formHandleEven">SRM 263</td>
        <td class="formHandleEven" align="right">1446</td>
    </tr>
--%>
<table width="180" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td><img src="/i/coder_month_bottom.gif" alt="" width="180" height="11" border="0"></td>
    </tr>
</table>
