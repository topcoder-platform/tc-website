...
/*				buf[buf_off++] = (byte) ((r - old_r)&0xFF);
        ...
        for (in_byte_off = 0; in_byte_off < buf.length; in_byte_off += DEFLATE_INPUT_SIZE) {
            ...
            while (!pixmapDeflater.needsInput()) {
                ...
                try {
                    writeFileHandle.write(deflate_buf, 0, deflate_size);
                } catch (IOException e) {
    ...
    public void imageComplete(int status) {
        ...
        if (!imageCompleteDone) {
            ...
            try {
                writeFileHandle.write(four_byte_buf);
            } catch (IOException e) {
                saveStatus = ImageObserver.ERROR;
            }
            ...
            // IEND type.
            try {
                writeFileHandle.write(IEND_CHUNK_TYPE);
            } catch (IOException e) {
                saveStatus = ImageObserver.ERROR;
            }
            ...
            try {
                ...
                System.out.println("IOException occurred opening RandomAccessFile : " + e);
            }
    ...
    private synchronized void writeHeader() {
        ...
        try {
            writeFileHandle.write(PNG_IDENTIFICATION_BYTES);
        } catch (IOException e) {
            saveStatus = ImageObserver.ERROR;
        }
        ...
        // IHDR type.
        try {
            writeFileHandle.write(IHDR_CHUNK_TYPE);
        } catch (IOException e) {
            saveStatus = ImageObserver.ERROR;
        }
        ...
        // Write out the data length. This is not known yet, so wa have to
        ...
        // IDAT type.
        try {
            writeFileHandle.write(IDAT_CHUNK_TYPE);
        } catch (IOException e) {
            saveStatus = ImageObserver.ERROR;
        }
