...
<xsl:for-each select="MAILDATA">
 The results are in from the design review of the <xsl:value-of select="PROJECT_NAME"/> Design.  Your design received a score of <xsl:value-of select="SCORE"/> points which was puts you in <xsl:value-of select="PLACE"/> place but did not get the minimum score.  I have attached the Review Scorecards from the architect review board for your reference.  If you have any questions, please feel free to send them to me.  In addition, I would like to thank you for participating and hope you will continue to submit design solutions for future projects.
