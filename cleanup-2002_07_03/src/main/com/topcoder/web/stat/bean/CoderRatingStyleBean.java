 ...
 *
 *           Revision 1.2  2002/03/13 02:49:44  tbone
 *           added javadoc comments
 *
 *           Revision 1.1  2002/02/26 04:46:25  tbone
 *           committed
 *
 *
 */
   ...
   */
    ...
    public int compareTo(Object obj) {
	  ...
	  if (obj instanceof CoderRatingBracket){
		...
		if (miMin > iVal) return miMax > iMaxVal ? (miMin-iVal):0;
		else if (miMin < iVal) return miMax < iMaxVal ? (miMin-iVal):0;
		else return 0;
	  }else if (obj instanceof MyNumber) {
	    iVal = ((MyNumber)obj).intValue();
		if (iVal < miMin) return miMin - iVal;
		else if (iVal > miMax) return miMax - iVal;
		else return 0;
	  }else throw new ClassCastException( obj.getClass().getName());
    }
  }
  ...
  private class MyNumber implements Comparable {
    ...
    public int compareTo(Object obj) {
	  ...
	  if (obj instanceof CoderRatingBracket){
	  	iVal = ((CoderRatingBracket)obj).miMin;
		int iMaxVal = ((CoderRatingBracket)obj).miMax;
		if (miNum >= iVal && (miNum <= iMaxVal)) return 0;
		else if (miNum < iVal) return -1;
		else return 1;
	  }else throw new ClassCastException( obj.getClass().getName());
    }
  }
