...
<form name="matchWinnersForm" action='<jsp:getProperty name="sessionInfo" property="servletPath"/>' method="get">
    ...
    <table class="stat" cellpadding="0" cellspacing="0" width="100%" border="0">
        <tbody>
            <tr>
                <td class="title" colspan="7">Marathon Match Winners</td>
            </tr>
            <tr>
                <td class="header"><a
                        href="${sessionInfo.servletPath}?<tc-webtag:sort column="<%= MatchWinners.DATE_COLUMN %>" includeParams="true" excludeParams="sr" />">Date</a>
                </td>
                <td class="header"><a
                        href="${sessionInfo.servletPath}?<tc-webtag:sort column="<%= MatchWinners.NAME_COLUMN %>" includeParams="true" excludeParams="sr" />">Match</a>
                </td>
                <td class="header"><a
                        href="${sessionInfo.servletPath}?<tc-webtag:sort column="<%= MatchWinners.PROBLEM_COLUMN %>" includeParams="true" excludeParams="sr" />">Problem</a>
                </td>
                <td class="header"><a
                        href="${sessionInfo.servletPath}?<tc-webtag:sort column="<%= MatchWinners.HANDLE_COLUMN %>" includeParams="true" excludeParams="sr" />">
                    Winner
                    <c:if test="${not ungrouped }">(s)</c:if>
                </a></td>
                <td class="header"><a
                        href="${sessionInfo.servletPath}?<tc-webtag:sort column="<%= MatchWinners.NUM_WINS_COLUMN %>" includeParams="true" excludeParams="sr" />">Wins</a>*
                </td>
            </tr>
            <c:forEach items="${list}" var="row" varStatus="status">
                ...
                <tr class='${status.index % 2 == 1? "dark" : "light" }'>
                    ...
                    <td class="value">
                        <mm:problemLink roundId="${row.map['round_id']}" problemId="${row.map['problem_id']}"
                    ...
                    <c:choose>
                        <c:when test="${ungrouped }">
                            <td class="value">
                                <tc-webtag:handle coderId="${row.map['coder_id']}" context="marathon_match"/>
                            </td>
                        ...
                        <c:when test="${fn:length(winnersMap[roundId]) > 3}">
                            <td class="value">
                                <div id="winnersShort${roundId}" class="shortList">
                                    <a href="javascript: hide('winnersShort${roundId}'); showBlock('winnersFull${roundId}'); showBlock('winsFull${roundId}');"
                                       onfocus="this.blur();">
                                ...
                                <div id="winnersFull${roundId}" class="fullList">
                                    ...
                                    <a href="javascript: hide('winnersFull${roundId}'); hide('winsFull${roundId}'); showBlock('winnersShort${roundId}');"
                                       onfocus="this.blur();">close</a>
                                </div>
                            </td>
                            <td class="valueR">
