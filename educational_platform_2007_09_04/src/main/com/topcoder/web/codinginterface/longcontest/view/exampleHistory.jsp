...
<td class="statTableSpacer" width="100%" valign="top">
    ...
    <span class="bigHandle">Contest: <mm:contestLink roundId="${infoRow.map['round_id']}"
                                                     name="${infoRow.map['display_name']}"/></span><br>
    ...
    <span class="bodySubtitle">Coder: <tc-webtag:handle context='marathon_match'
                                                        coderId='<%=request.getParameter(Constants.CODER_ID)%>'/></span>
    <br/>
            ...
            <tr>
                <td>
                    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="statTable">
                        ...
                        <tr>
                            <td class="tableTitle" colspan="<%= over || self? 4 : 3 %>">Example
                                Tests: ${infoRow.map['num_submissions']}</td>
                        </tr>
                        <tr>
                            <td class="tableHeader" align="center" nowrap="nowrap">
                                <A href="<%=sessionInfo.getServletPath()%>?<tc-webtag:sort column="<%=examples.getColumnIndex("submission_number")%>" includeParams="true" excludeParams="sr;nr"/>">Example
                                    Test</A></td>
                            <td class="tableHeader" width="100%" align="center">
                                <A href="<%=sessionInfo.getServletPath()%>?<tc-webtag:sort column="<%=examples.getColumnIndex("submit_time")%>" includeParams="true" excludeParams="sr;nr"/>">Time</A>
                            </td>
                            <td class="tableHeader" align="center" nowrap="nowrap">
                                <A href="<%=sessionInfo.getServletPath()%>?<tc-webtag:sort column="<%=examples.getColumnIndex("language_name")%>" includeParams="true" excludeParams="sr;nr"/>">Language</A>
                            </td>
                            <% if (over || self) { %>
                        ...
                        <rsc:iterator list="<%=examples%>" id="resultRow">
                            <tr>
                                <td class="<%=even?"statLt":"statDk"%>" align="center">
                                    <% if (over || self) { %>
                                    <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=Constants.MODULE%>=ViewProblemSolution&amp;<%=Constants.PROBLEM_ID%>=<rsc:item name="problem_id" row="<%=resultRow%>"/>&amp;<%=Constants.ROUND_ID%>=<rsc:item name="round_id" row="<%=resultRow%>"/>&amp;<%=Constants.CODER_ID%>=<rsc:item name="coder_id" row="<%=resultRow%>"/>&amp;<%=Constants.SUBMISSION_NUMBER%>=<rsc:item name="submission_number" row="<%=resultRow%>"/>&amp;<%=Constants.EXAMPLE_FLAG%>=1"
                                       class="statLink">
                                        <% } %>
                                ...
                                <td class="<%=even?"statLt":"statDk"%>" align="center" nowrap="nowrap">
                                    <tc-webtag:format object="<%=new Date(resultRow.getLongItem("submit_time"))%>"
                                                      format="MM.dd.yyyy HH:mm:ss"/>
                                    <%=resultRow.getIntItem("status_id") == 130 ? "*" : ""%>
                                </td>
                                <td class="<%=even?"statLt":"statDk"%>" align="center">
                                ...
                                <td class="<%=even?"statLt":"statDk"%>" align="center" nowrap="nowrap">
                                    ...
                                    <A href="<jsp:getProperty name="sessionInfo" property="servletPath"/>?<%=Constants.MODULE%>=ViewExampleResults&amp;<%=Constants.PROBLEM_ID%>=<rsc:item name="problem_id" row="<%=resultRow%>"/>&amp;<%=Constants.ROUND_ID%>=<rsc:item name="round_id" row="<%=resultRow%>"/>&amp;<%=Constants.CODER_ID%>=<rsc:item name="coder_id" row="<%=resultRow%>"/>"
                                       class="statLink">example results</A>
                                    <% } %>
                                </td>
                                <% } %>
                            </tr>
