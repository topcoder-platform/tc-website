...
<strong>Record:</strong> The competitors who have achieved the highest ratings based on the results of their first match.
    ...
    <table class="stat" cellpadding="0" cellspacing="0"
        ...
        <thead>
            <tr>
                <td class="title" colspan="5">Impressive Debuts</td>
            </tr>
            <tr>
            ...
            <c:forEach items="${results}" var="row" varStatus="status">
                <tr class='${status.index % 2 == 1? "dark" : "light" }'>
                    ...
                    <td class="value" nowrap="nowrap">
                        <mm:contest roundTypeId="${row.map['round_type_id']}" contestName="${row.map['contest_name']}"
                                    roundName="${row.map['round_name']}"/>
                    </td>
                    <td class="valueC">
                        <fmt:formatDate value="${row.map['date']}" pattern="MM/dd/yyyy"/>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
