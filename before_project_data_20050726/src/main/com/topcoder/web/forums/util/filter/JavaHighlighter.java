...
public class JavaHighlighter implements Filter {
     ...
     */
    private com.jivesoftware.util.JavaCodeViewer viewer;

    /**
     * Construct a new JavaHighlighter filter.
     */
    public JavaHighlighter() {
        viewer = new com.jivesoftware.util.JavaCodeViewer();
    ...
    public String applyFilter(String string, int currentIndex, FilterChain chain) {
        ...
        // short circuit this filter if no [java] found
        if (string.indexOf("[java]") < 0) {
            return chain.applyFilters(currentIndex, string);
        }

        // we have something to filter
        while ((startCodeTag = string.indexOf("[java]", endCodeTag)) >= 0) {
            int end = string.indexOf("[/java]", startCodeTag + 6);

            if (end > 0) {
                if (endCodeTag < startCodeTag) {
                    // apply filters for content between [/java] and [java]
                ...
                filtered.append(string.substring(endCodeTag, startCodeTag) + "[java]");
            ...
            if (applyTableSurround) {
                filtered.append(surroundWithTable(viewer.javaCodeFilter(code), countLines(code)));
            }
            else {
                filtered.append("<pre>");
                filtered.append(viewer.javaCodeFilter(code));
                filtered.append("</pre>");
            }
        }
